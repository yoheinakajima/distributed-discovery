...
`make site` builds four pages and provenance data, then validates internal links, semantic landmarks and heading order, resolved data markers, and tracking absence. Integration tests also verify generated content and primary light/dark text contrast. M4 originally produced a local-only artifact; the source is now public by explicit project-owner decision and the generated output deploys from `main` through GitHub Actions.
