...
Dependency: M1 correlation benchmark and M2 terminology. Enumerate every model-valid bipartite graph with four searchers and one to three nonisolated sources at fixed accuracy \(2/3\). Canonicalize under source/searcher relabeling, independently verify orbit counts, enumerate all target/source-signal states exactly, and group graphs by the complete first/pairwise observable report law. Search each group for different private discovery; preserve either an exact paired witness or the complete bounded null.
