...
independent source raises net social value. This paper studies that wedge in a
...
enumeration for $N=2,\ldots,8$ reproduces the formulas. The result is not a universal under-acquisition theorem:
at $N=3$, $p=4/5$, and $c=13/375$, the
...
claiming human evidence. Its finite grids and analytic curves are generated
from checksum-validated immutable artifacts; companion figures reproduce
separately registered, non-nested models.
\end{abstract}
...
theorems (DD-C-0057). The two-agent trap interval is an independently reproduced exact bounded-grid result (DD-C-0051), while the
$N=2,\ldots,8$ tables are exact finite classifications (DD-C-0052). The
interior over-acquisition fixture is an exact negative result (DD-C-0058).
DD-011 contributes synthetic predictions only; no participant was recruited
and no human experiment was conducted.}
...
The first result is an all-finite-$N$ threshold characterization for the
homogeneous equal-prize model. If $k$ agents currently use independent sources,
a common user compares its shared
...
\input{generated/trap-region.tex}
...
belongs only to the frozen homogeneous equal-prize source-choice game. This
paper therefore treats its residual mathematical novelty as conditional: a
targeted review can establish adjacency, but it cannot exclude an equivalent
finite-count result under different terminology.
...
predate this paper. The paper's claimed contribution is only the exact theorem
for the declared coverage-and-prize model; the project makes no verified
novelty claim.}
...
Only the Bernoulli event that a source is correct enters the core payoff and
coverage formulas. The three-target, symmetric-error specification fixes the
companion mechanism examples, but the source-count theorem itself does not use
the number or relative probabilities of incorrect labels.
...
For $0<k\leq N$, an independent user's gross payoff is
\begin{equation}
...
At $k=N$, the two conditional shares coincide and the expression collapses to
...
The name ``Common-Source Trap'' refers to this exact coexistence of a
common-source equilibrium and a planner preference for at least one independent
...
acquisition. At larger $N$, the count cannot be inferred from the all-common
boundary alone; the interior thresholds and prize-sharing comparisons remain
necessary.

\input{generated/planner-counts.tex}
...
The ordering is not immediate: adding an independent user changes both the
binomial winner count and the size of the common block. The coupling below
holds those changes together and exposes a strictly positive difference kernel.
...
two adjacent source-count profiles. Strict threshold ordering implies that at
least one weak equilibrium count exists and that it is unique except at a
threshold, where two adjacent counts are weak equilibria.
...
For $k=0,\ldots,N-2$, set $m=N-k\geq2$. Equation \eqref{eq:ak} and the
coupling give the exact connection
\[
 A_k-A_{k+1}
 =pq\E\left[f_m(X_k)-f_{m-1}(X_k+Y)\right].
\]
...
\eqref{eq:ak} separately gives the terminal value $A_{N-1}=0$. For the
smallest team, this reduces to the hand check $A_0=pq/2>A_1=0$.
...
checks audit transcription and implementation; they do not replace the
positive-kernel proof.
...
threshold from below. For every fixed interior $k\geq1$, however,
\begin{equation}
 1-q^{k+1}=p\sum_{j=0}^{k}q^j
 >p(k+1)q^k,
 \label{eq:large-n-reversal}
\end{equation}
because $0<q<1$ and every term before the last strictly exceeds $q^k$.
Equations \eqref{eq:large-n} and \eqref{eq:large-n-reversal} imply
\(\lim_{N\to\infty}A_k>pq^{k+1}=B_k\). Thus, for each fixed $p\in(0,1)$
and $k\geq1$, the private margin eventually exceeds the corresponding planner
margin as $N$ grows. This is a margin-by-margin asymptotic statement; it does
not claim a common finite-$N$ cutoff across margins, empirical prevalence, or a
universal ordering of equilibrium and planner source counts.
...
ordered threshold sequences. The source-choice comparison is encoded by those
sequences; the economic content lies in their differences.
...
The reversal is driven by rent competition, showing that the acquisition
externality does not have a fixed sign across interior margins. Moving from one
to two independent users changes how a prospective buyer shares the prize with
common and independent winners. When accuracy is
...
example is the smallest registered finite-team witness of the broader
fixed-margin asymptotic reversal in equation \eqref{eq:large-n-reversal}. It
does not establish empirical prevalence, a reversal at every finite $N$, or a
universal policy direction. It shows that the phrase ``the Common-Source Trap''
cannot stand for a universal equilibrium ordering at every interior count.
...
\input{generated/independence-gap.tex}
...
\input{generated/discovery-gap.tex}
...
\input{generated/welfare-gap.tex}
...
private condition to $c-s<A_0$. Strict inducement requires $s>c-A_0$, so the
infimum required subsidy is $c-A_0$; no smallest strict subsidy exists.
Inducing exactly one purchase additionally requires $c-s\geq A_1$, whereas
$s>c-A_0$ establishes only that at least one purchase is privately profitable.
If the planner wants the first independent source,
...
Assignment controls the source count more directly. A planner can designate
exactly $k^\star$ independent users and thereby attain the source-count optimum without changing
...
\input{generated/interventions.tex}
...
This and the next two sections are a companion-model map, not additional
evidence for the source-count theorem. They preserve registered qualitative
implications and bounded outputs, while their primitives and estimands remain
those of DD-006B, DD-009, and DD-011 rather than the core model above.
...
registered normalized class, not a dominant-strategy theorem or an
arbitrary-transfer characterization.
...
\input{generated/atlas-slice.tex}

The acquisition slice in Figure \ref{fig:atlas-slice} holds two independent
channels and total information cost $1/4$ across both channels fixed. Direct
private action yields discovery
...
Figure \ref{fig:atlas-slice}'s ``net value'' follows the registered DD-009
Atlas convention: discovery minus total information cost minus any external
transfer budget. Thus A210 subtracts its $7/4$ DD-006B subsidy, whereas
equation \eqref{eq:welfare} treats the core unit prize as an internal transfer.
The columns are exact within their respective models but are not one
commensurable welfare ranking.

Reward changes can also alter accounting without changing discovery. A
sole-rescue or marginal-coverage transfer can use budget while preserving the same
...
The Atlas makes favorable cherry-picking within its declared architecture space
more visible. A claim that ``independent evidence plus pooling works'' must
specify the allocation rule. A claim that a
...
DD-011 translates the project's exact mechanisms into a candidate synthetic preregistration package.
The selected design has 20 treatment cells spanning
acquisition, attribution, disclosure, action timing, and rewards. It registers
eight hypotheses---six primary and two secondary---fifteen outcomes, eight
explicit synthetic response scenarios, and six sample sizes. A seeded
384,000-draw Monte Carlo run reports conditional power
and minimum detectable effects; a separate verifier recreates every random
stream (DD-C-0056). Every reported power row already uses the registered conservative Bonferroni family alpha, $0.05$ divided by the size of its frozen
multiplicity family; the grid is not an unadjusted per-hypothesis
$\alpha=0.05$ analysis.
...
treatment effects. Under its favorable rational synthetic scenario, all eight
assumed effects reach estimated power at least $0.80$ by total sample size 960.
Separately, a retained calibration failure is a registered
scenario--hypothesis row whose estimated power remains below $0.80$ at a
registered sample size of 640, 800, or 960. Across all scenarios, 140 of those
192 large-sample rows fail that criterion. Those failures diagnose sensitivity
of the synthetic assumptions; they neither estimate human power nor establish
that a real experiment is infeasible.
...
before deployment. The materials are an unsubmitted candidate preregistration
package, not a preregistration, an externally reviewed protocol, or a claim of
empirical power. Figure \ref{fig:experiment-map}'s inherited
``preregistration-ready'' label denotes the presence of registered materials,
not an owner decision that the design is ready to deploy.}
...
The Common-Source Trap has a precise all-common boundary in the frozen model.
In that finite-team model, a prospective independent-source buyer compares two
expected reciprocal
...
not remain below planner thresholds at every interior count. At the first-source
margin, the all-common trap interval has width $p(1-p)/N$ and shrinks with team
size. At every fixed interior margin, the private threshold instead eventually
exceeds the corresponding planner threshold as the team grows. The exact
$N=3$, $p=4/5$ fixture is the smallest registered finite witness of that
asymptotic reversal. Any policy or empirical design built from the theorem must
therefore measure the relevant margin rather than assume that ``more independent
sources'' is always the corrective direction.
...
each intervention at the layer it changes. Under that discipline, the
Common-Source Trap is an auditable mapping from private source choice to collective
discovery, with an exact all-common boundary and an explicit interior
counterboundary.
...
most direct empirical extension uses DD-011's candidate preregistration cells after
