#!/usr/bin/env python3
"""Audit AO-0006 registration, corruptions, and the full offline rehearsal."""
...
REHEARSAL = ROOT / "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v2-offline-rehearsal.yml"
CORRUPTIONS = ROOT / "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v2-corruptions.yml"
...
def audit(*, rehearsal: bool = False) -> dict[str, object]:
    ...
    if set(by_id) != set(required_ids):
        raise ValueError("v2 corruption implementation and registry differ")
    ordered = [by_id[corruption_id] for corruption_id in required_ids]
    if any(item["status"] != "rejected" for item in ordered):
        raise ValueError("a v2 corruption was accepted")
    ...
    if committed_corruptions["corruptions"] != ordered:
        raise ValueError("committed v2 corruption report is stale")

    result: dict[str, object] = {
        **registration,
        "v2_boundary_corruptions_rejected": len(ordered),
    ...
    if rehearsal:
        ...
        if _load(REHEARSAL) != full:
            raise ValueError("committed v2 full rehearsal report is stale")
