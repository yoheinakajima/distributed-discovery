#!/usr/bin/env python3
"""Audit AO-0010 registration, corruptions, and the full offline rehearsal."""
...
REHEARSAL = ROOT / "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v4-offline-rehearsal.yml"
CORRUPTIONS = ROOT / "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v4-corruptions.yml"
CUSTODY = (
    ROOT
    / "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v4-production-custody-rehearsal.yml"
)
SCENARIOS = (
    ROOT
...
def audit(*, rehearsal: bool = False) -> dict[str, object]:
    ...
    if set(by_id) != set(required_ids):
        raise ValueError("v4 corruption implementation and registry differ")
    ordered = [by_id[corruption_id] for corruption_id in required_ids]
    if any(item["status"] != "rejected" for item in ordered):
        raise ValueError("a v4 corruption was accepted")
    ...
    if committed_corruptions["corruptions"] != ordered:
        raise ValueError("committed v4 corruption report is stale")

    result: dict[str, object] = {
        **registration,
        "v4_boundary_corruptions_rejected": len(ordered),
    ...
    if rehearsal:
        ...
        if _load(REHEARSAL) != full:
            raise ValueError("committed v4 full rehearsal report is stale")
        custody = run_production_permit_custody_rehearsal(ROOT)
        if _load(CUSTODY) != custody:
            raise ValueError("committed v4 production custody rehearsal report is stale")
        scenarios = run_protocol_validity_scenarios(ROOT)
        if _load(SCENARIOS) != scenarios:
            raise ValueError("committed v4 protocol-validity scenario report is stale")
        result["rehearsal"] = full
        result["production_custody_rehearsal"] = custody
        result["protocol_validity_scenarios"] = scenarios
    return result
