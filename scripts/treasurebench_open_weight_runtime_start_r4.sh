...
required=(
  ...
  TREASUREBENCH_EXPECTED_NAMESPACE
  TREASUREBENCH_RUNTIME_MANIFEST_B64
  TREASUREBENCH_PROXY_SCRIPT_B64
  TREASUREBENCH_WATCHDOG_SCRIPT_B64
)
for name in "${required[@]}"; do
  [[ -n "${!name:-}" ]] || die "missing-${name}"
done

runtime_dir=/run/treasurebench-runtime-r4
model_dir=/workspace/model
manifest_path="${runtime_dir}/manifest.yml"
proxy_path="${runtime_dir}/proxy.py"
watchdog_path="${runtime_dir}/watchdog.py"
watchdog_status_path="${runtime_dir}/watchdog-status.json"
...
printf '%s' "${TREASUREBENCH_WATCHDOG_SCRIPT_B64}" | base64 --decode >"${watchdog_path}"
chmod 600 "${manifest_path}" "${proxy_path}" "${watchdog_path}"
unset TREASUREBENCH_RUNTIME_MANIFEST_B64 TREASUREBENCH_PROXY_SCRIPT_B64
unset TREASUREBENCH_WATCHDOG_SCRIPT_B64
...
if manifest["schema_version"] != "treasurebench-open-weight-cloud-runtime-manifest-r4-v1":
    raise SystemExit("R4 manifest required")
...
# Arm and verify the Pod-resident delete watchdog before download or engine work.
env -i \
  PATH="${PATH}" \
  HOME=/root \
  RUNPOD_API_KEY="${RUNPOD_API_KEY}" \
  RUNPOD_POD_ID="${RUNPOD_POD_ID}" \
  TREASUREBENCH_EXPECTED_NAMESPACE="${TREASUREBENCH_EXPECTED_NAMESPACE}" \
  TREASUREBENCH_WATCHDOG_STATUS_PATH="${watchdog_status_path}" \
  python3 "${watchdog_path}" >/dev/null 2>&1 &
watchdog_pid=$!
watchdog_ready=0
for _ in $(seq 1 30); do
  kill -0 "${watchdog_pid}" 2>/dev/null || die "watchdog-self-query"
...
[[ "${watchdog_ready}" -eq 1 ]] || die "watchdog-readiness-timeout"
python3 - "${watchdog_status_path}" "${watchdog_pid}" <<'PY'
import json
import pathlib
import sys
...
if path.is_symlink() or not path.is_file() or path.stat().st_mode & 0o077:
    raise SystemExit("watchdog status boundary mismatch")
...
if value != {
    ...
    raise SystemExit("watchdog attestation mismatch")
PY
unset RUNPOD_API_KEY RUNPOD_POD_ID TREASUREBENCH_EXPECTED_NAMESPACE
...
# The provider-injected Pod key is held only by the watchdog, never by either child.
...
python3 -   "${manifest_path}" "${attestation_path}" "${artifact_path}"   "${download_started}" "${download_finished}" "${load_started}" "${load_finished}"   "${mistral_common_version}" "${peak_path}" "${watchdog_status_path}" <<'PY'
import ctypes
import hashlib
import hmac
...
watchdog = json.load(open(sys.argv[10], encoding="utf-8"))
if watchdog.get("verified") is not True or watchdog.get("deadline_seconds") != 21600:
    raise SystemExit("watchdog attestation unavailable")
try:
    os.kill(int(watchdog["pid"]), 0)
except (KeyError, OSError, TypeError, ValueError):
    raise SystemExit("watchdog process unavailable") from None
record = {
    ...
    "watchdog_deadline_seconds": int(watchdog["deadline_seconds"]),
}
