...
Status: registered offline instrument specification; not executed.
...
This directory freezes contracts and public conformance fixtures only. No
provider/model call, model download, cost, credential, private seed, holdout,
answer key, evaluated trace, result, claim, or immutable run exists.

Future execution requires a separate issue and registration, accepted offline
implementation, exact provider/model snapshots, explicit owner cost
...
- 15 JSON schemas, clearly public toy fixtures and commitments, a deterministic
  no-network mock, and the offline audit command `make audit-agents-v1`.

The authoritative outcome is
`reports/roadmap-consolidation/discoverybench-agents-v1-registration-decision.yml`:
`register-instrument-under-dd010`.
