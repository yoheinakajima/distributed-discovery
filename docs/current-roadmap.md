...
| DD-018 Minimum Viable Team Mechanisms | active registered baseline on issue #113 | Freeze and test the ten-mechanism common-posterior `M=3`, `N=4`, `tau=2` census; unilateral implementation is not coalition-proofness, and no result exists before the clean primary run. |
