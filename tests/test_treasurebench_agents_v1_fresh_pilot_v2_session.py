...
def test_r2_success_control_flow_runs_through_merge_deploy_close_and_main_sync() -> None:
    ...
    assert hooks.events.index("publish-output-lock") < hooks.events.index("protocol-contract")
...
def test_r2_every_registered_failure_quarantines_then_completes_admin_closeout(
    failure_point: str,
) -> None:
    ...
    assert result["decision"] == "fresh-pilot-v2-quarantined-engineering-only"
...
def test_r2_unsafe_retained_state_returns_to_owner_without_merge() -> None:
...
def test_r2_nonroutine_authority_or_execution_surface_change_stops_before_merge(
    stop_point: str,
) -> None:
