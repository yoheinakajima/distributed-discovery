...
from distributed_discovery.benchmark.agents_v1 import fresh_pilot_v4, fresh_pilot_v4_live
...
def test_v4_registration_allocation_budget_and_provider_boundary_are_exact() -> None:
    registration = fresh_pilot_v4.validate_registration(ROOT)
    slots = fresh_pilot_v4.allocation_slots(ROOT)
    tasks = fresh_pilot_v4.generate_tasks(
        ROOT,
        material="FRESH-RC-V4-TEST-PUBLIC",
        public_fixture=True,
    )
    boundary = fresh_pilot_v4.validate_provider_boundary(tasks)
    ...
    assert all(slot.slot_id.startswith("RCV4-SLOT-") for slot in slots)
...
def test_v4_private_generation_fails_before_generic_owner_authorization() -> None:
    with pytest.raises(PermissionError, match="owner authorization"):
        fresh_pilot_v4.generate_tasks(ROOT, material="forbidden", public_fixture=False)


def test_v4_production_permit_and_custody_rehearsal_uses_no_synthetic_bypass() -> None:
...
def test_v4_complete_500_run_rehearsal() -> None:
    result = fresh_pilot_v4.run_synthetic_rehearsal(ROOT)
    assert result["status"] == "pass"
    assert result["tasks"] == 50
...
def test_all_76_v4_corruptions_reject_and_match_registry() -> None:
    observed = (
        *fresh_pilot_v4.audit_corruptions(ROOT),
        *audit_live_corruptions(ROOT),
    )
    registry = fresh_pilot_v4.load_corruption_registry(ROOT)
...
def test_v4_route_token_caps_fail_closed(
    tmp_path: Path,
    provider: str,
    input_tokens: int,
    output_tokens: int,
...
def test_v4_offline_module_has_no_credential_or_provider_runtime() -> None:
    source = (ROOT / "src/distributed_discovery/benchmark/agents_v1/fresh_pilot_v4.py").read_text(
        encoding="utf-8"
    )
...
def _synthetic_generic_authorization() -> dict[str, object]:
    request = fresh_pilot_v4.load_request(ROOT)
    return {
        "schema_version": "agent-ops-owner-authorization-v1",
        "kind": "owner-authorization",
        "synthetic": True,
        "gate_id": fresh_pilot_v4.GATE_ID,
        "issue": fresh_pilot_v4.ISSUE,
        "pull_request": 0,
        "branch": fresh_pilot_v4.BRANCH,
        "commit": request["starting_main"],
        "task_contract_sha256": "sha256:" + "0" * 64,
        "tree_hashes": fresh_pilot_v4.execution_tree_hashes(ROOT),
...
def test_v4_live_driver_uses_distinct_private_root() -> None:
    path = private_state_root().as_posix()
    assert path.endswith("/treasurebench-agents-v1/repair-confirmation-v4")
    assert "repair-confirmation-v1" not in path


def test_v4_live_driver_stops_before_authorization_and_credentials(
...
def test_v4_staged_mock_driver_completes_and_resumes_without_calls(tmp_path: Path) -> None:
    root = tmp_path / "fresh-v4-mock-live"
    ...
    assert first["campaign_id"] == fresh_pilot_v4.CAMPAIGN_ID
    assert first["batch_id"] == fresh_pilot_v4.BATCH_ID
    assert first["private_runs"] == 500
    assert first["method_a_b_disagreements"] == 0
    assert first["method_c_classification_failures"] == 0
    assert first["metric_bound_disagreements"] == 0
    assert first["protocol_valid_pairings"] == 500
    assert first["protocol_invalid_pairings"] == 0
    assert first["provider_terminal_missing_pairings"] == 0
    assert first["terminal_pairings_classified"] == 500
    assert first["protocol_invalid_alone_quarantined"] is False
    assert first["independent_metric_bound_agreement"] is True
    assert first["identity_correspondence"]["exact"] is True
    ...
    assert first["no_call_after_lock"] is True
    assert first["unseal_after_lock_verified"] is True
    assert first["post_lock_derived_objects"] == 1000
    assert str(first["post_lock_derived_state_commitment"]).startswith("sha256:")
...
def test_v4_zero_mixed_and_all_invalid_scenarios_preserve_denominators() -> None:
    result = fresh_pilot_v4.run_protocol_validity_scenarios(ROOT)
    assert result["status"] == "pass"
    assert result["first_middle_final_invalid_tested"] is True
    assert result["protocol_invalid_alone_quarantines"] is False
    assert result["provider_terminal_missing_quarantines"] is True
    assert result["pairing_integrity_failure_quarantines"] is True
    assert result["complete_case_overclaim_rejected"] is True
    scenarios = result["scenarios"]
    assert scenarios["zero-invalid"]["protocol_invalid"] == 0
    assert scenarios["mixed-invalid-first-middle-final"]["protocol_invalid"] == 3
    assert scenarios["all-invalid"]["protocol_invalid"] == 500
    for scenario in scenarios.values():
        assert scenario["intended_pairings"] == 500
        assert scenario["quarantined"] is False
        assert scenario["replacement_pairings"] == 0
        assert scenario["semantic_answer_retries"] == 0
        assert scenario["constructed_or_credited_invalid_actions"] == 0
        assert scenario["independent_bound_agreement"] is True


def test_v4_invalid_output_separates_no_action_credit_from_feasible_bounds() -> None:
    task = fresh_pilot_v4.generate_tasks(
        ROOT,
        material="FRESH-RC-V4-INVALID-BOUND-TEST",
        public_fixture=True,
    )[0]
    classification = classify_completed_pairing(
        pairing_id="invalid-bound-test",
        model=fresh_pilot_v4.MODELS[0],
        task_commitment=task.commitment,
        architecture_id="isolated-private-agents",
        trace_id="invalid-bound-test-trace",
    ...
    discovery = by_metric["group-discovery"]
    assert discovery.operational_credit == "0-no-valid-action-credit"
    assert str(discovery.lower) == "0"
    assert str(discovery.upper) == "1"
    assert discovery.exact_metric_defined is False
    assert by_metric["protocol-compliance"].exact_metric_defined is True
    assert str(by_metric["protocol-compliance"].lower) == "0"
    assert by_metric["invalid-action-rate"].exact_metric_defined is True
    assert str(by_metric["invalid-action-rate"].lower) == "1"
    assert by_metric["calls"].exact_metric_defined is True
    assert by_metric["tokens"].exact_metric_defined is True
    assert by_metric["cost"].exact_metric_defined is True


def _policy_runner_adapters(
    tmp_path: Path,
    ledger: AppendOnlyLedger,
    *,
    first_mode: str,
) -> tuple[dict[str, object], dict[str, MockAdapter]]:
    response_key = b"r" * 32
    underlying = {
        fresh_pilot_v4.MODELS[0]: (
            MockAdapter("scripted", script=("{}",))
            if first_mode == "protocol-invalid"
            else MockAdapter(first_mode)
        ),
        fresh_pilot_v4.MODELS[1]: MockAdapter(),
    }
    adapters = {
        model: fresh_pilot_v4_live._resumable(
            underlying=underlying[model],
            provider=fresh_pilot_v4.PROVIDERS[index],
            model=model,
            ledger=ledger,
            response_root=tmp_path / "responses" / fresh_pilot_v4.PROVIDERS[index].lower(),
            response_key=response_key,
        )
        for index, model in enumerate(fresh_pilot_v4.MODELS)
    }
    return adapters, underlying


def test_v4_runner_retains_protocol_invalid_pairings_without_replacement(
    tmp_path: Path,
) -> None:
    ledger = AppendOnlyLedger(tmp_path / "ledger.jsonl")
    adapters, underlying = _policy_runner_adapters(
        tmp_path,
        ledger,
        first_mode="protocol-invalid",
    )
    task = fresh_pilot_v4.generate_tasks(
        ROOT,
        material="FRESH-RC-V4-PROTOCOL-INVALID-TEST",
        public_fixture=True,
    )[0]
    runner = fresh_pilot_v4_live._runner(tmp_path, ledger, b"k" * 32)
    prelock = runner.run_stage(
    ...
    assert prelock["analysis"] == "deferred-until-post-lock-unseal"
    assert prelock["raw_traces_persisted"] == 10
    assert not tuple((tmp_path / "encrypted-protocol-classifications").glob("*.sealed"))
    assert not tuple((tmp_path / "encrypted-metric-bounds").glob("*.sealed"))
    calls_before_lock = sum(record["event_type"] == "provider-call" for record in ledger.records)
    ...
    assert result["terminal_pairings_classified"] == 10
    assert result["protocol_valid"] == 5
    assert result["protocol_invalid"] == 5
    assert result["provider_terminal_missing"] == 0
    assert result["protocol_invalid_alone_quarantines"] is False
    assert result["independent_bound_agreement"] is True
    assert len(tuple((tmp_path / "encrypted-traces").glob("*.sealed"))) == 10
    assert len(tuple((tmp_path / "encrypted-protocol-classifications").glob("*.sealed"))) == 10
    assert len(tuple((tmp_path / "encrypted-metric-bounds").glob("*.sealed"))) == 10
    assert (
        sum(record["event_type"] == "provider-call" for record in ledger.records)
        == calls_before_lock
    )
    invalid_adapter = underlying[fresh_pilot_v4.MODELS[0]]
    assert invalid_adapter.calls == sum(
        2
        for record in ledger.records
        if record["event_type"] == "provider-call"
        and record["model"] == fresh_pilot_v4.MODELS[0]
        and record["schema_retry"] is False
    )


def test_v4_runner_provider_terminal_missing_stops_before_later_calls(
    tmp_path: Path,
) -> None:
    ledger = AppendOnlyLedger(tmp_path / "ledger.jsonl")
    adapters, underlying = _policy_runner_adapters(
    ...
    task = fresh_pilot_v4.generate_tasks(
        ROOT,
        material="FRESH-RC-V4-PROVIDER-TERMINAL-TEST",
        public_fixture=True,
    )[0]
    runner = fresh_pilot_v4_live._runner(tmp_path, ledger, b"k" * 32)
    with pytest.raises(ProviderTerminalMissingError, match="provider-terminal-missing"):
        runner.run_stage(
            stage="private-prefix",
            tasks=(task,),
            adapters=adapters,
    ...
    assert underlying[fresh_pilot_v4.MODELS[0]].calls == 1
    assert underlying[fresh_pilot_v4.MODELS[1]].calls == 0
...
def test_v4_live_adapters_request_return_and_retain_only_exact_two_credentials(
    tmp_path: Path,
    ...
    monkeypatch.setattr(fresh_pilot_v4_live, "OpenAIResponsesAdapter", capture_openai)
    monkeypatch.setattr(fresh_pilot_v4_live, "AnthropicMessagesAdapter", capture_anthropic)
    ledger = RouteCappedLedger(tmp_path / "ledger.jsonl")
    _, credentials, underlying = fresh_pilot_v4_live._live_adapters(
        repo,
    ...
    fresh_pilot_v4_live._clear_stage_secrets(credentials, underlying)
...
def test_v4_credential_source_refuses_before_adapter_or_call(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    case: str,
) -> None:
    ...
    monkeypatch.setattr(fresh_pilot_v4_live, "OpenAIResponsesAdapter", prohibit_adapter)
    monkeypatch.setattr(fresh_pilot_v4_live, "AnthropicMessagesAdapter", prohibit_adapter)
    with pytest.raises(PermissionError):
        fresh_pilot_v4_live._live_adapters(
            repo,
            _synthetic_generic_authorization(),
...
def test_v4_preflight_failures_precede_credential_ingress(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    failure_point: str,
) -> None:
    ...
    monkeypatch.setattr(fresh_pilot_v4_live, "load_credentials", prohibit_credentials)
    monkeypatch.setattr(
        fresh_pilot_v4_live,
    ...
    with pytest.raises(PermissionError, match=expected):
        fresh_pilot_v4_live._prepare_live_stage(ROOT)
...
def test_v4_every_provider_stage_and_exit_path_clears_selected_secrets(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    stage: str,
    outcome: str,
    ...
    credentials = CredentialSet(
        ...
        allowed_names=fresh_pilot_v4_live.CREDENTIAL_NAMES,
        configured={name: True for name in fresh_pilot_v4_live.CREDENTIAL_NAMES},
        unused_present=(),
    )
    adapters = (
    ...
    ledger = RouteCappedLedger(tmp_path / "ledger.jsonl")
    prepared = fresh_pilot_v4_live.PreparedLiveStage(
        authorization=_synthetic_generic_authorization(),
        root=tmp_path / "state",
    ...
    monkeypatch.setattr(fresh_pilot_v4_live, "_prepare_live_stage", lambda repo: prepared)
    monkeypatch.setattr(
        fresh_pilot_v4_live,
        "_live_adapters",
    ...
    monkeypatch.setattr(fresh_pilot_v4_live, "_execute_stage", execute)
    if outcome == "success":
        assert fresh_pilot_v4_live._run_prepared_live_stage(ROOT, prepared)["status"] == "pass"
    else:
        with pytest.raises(RuntimeError, match=outcome):
            fresh_pilot_v4_live._run_prepared_live_stage(ROOT, prepared)
    assert credentials.get_secret("OPENAI_API_KEY") is None
    assert credentials.get_secret("ANTHROPIC_API_KEY") is None
...
def _prepared_stage(tmp_path: Path, stage: str) -> fresh_pilot_v4_live.PreparedLiveStage:
    return fresh_pilot_v4_live.PreparedLiveStage(
        authorization=_synthetic_generic_authorization(),
        root=tmp_path / "state",
        operational_key=b"k" * 32,
        response_root=tmp_path / "responses",
...
def test_v4_live_command_runs_all_stages_and_public_commitments_without_pause(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    stages = iter(
    ...
    monkeypatch.setattr(fresh_pilot_v4_live, "_run_prepared_live_stage", run_stage)
    monkeypatch.setattr(
        fresh_pilot_v4_live,
...
def test_v4_live_terminal_failure_quarantines_and_never_advances(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    def quarantine(
        value: fresh_pilot_v4_live.PreparedLiveStage,
        *,
        failure_class: str,
    ) -> dict[str, object]:
    ...
    monkeypatch.setattr(fresh_pilot_v4_live, "_quarantine_live_failure", quarantine)
    monkeypatch.setattr(
        fresh_pilot_v4_live,
        "_publish_public_commitment",
...
def test_v4_live_cap_guard_quarantines_before_credential_ingress(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda repo: (_ for _ in ()).throw(
            fresh_pilot_v4_live.PreparedStageFailure(
                prepared=prepared,
...
def test_v4_live_output_lock_failure_stops_for_owner(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
