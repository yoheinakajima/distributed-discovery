...
from distributed_discovery.benchmark.agents_v1 import fresh_pilot_v5, fresh_pilot_v5_live
...
def test_v5_registration_allocation_budget_and_provider_boundary_are_exact() -> None:
    registration = fresh_pilot_v5.validate_registration(ROOT)
    slots = fresh_pilot_v5.allocation_slots(ROOT)
    tasks = fresh_pilot_v5.generate_tasks(
        ROOT,
        material="FRESH-RC-V5-TEST-PUBLIC",
        public_fixture=True,
    )
    boundary = fresh_pilot_v5.validate_provider_boundary(tasks)
    ...
    assert all(slot.slot_id.startswith("RCV5-SLOT-") for slot in slots)
...
def test_v5_private_generation_fails_before_generic_owner_authorization() -> None:
    with pytest.raises(PermissionError, match="owner authorization"):
        fresh_pilot_v5.generate_tasks(ROOT, material="forbidden", public_fixture=False)


def test_v5_production_permit_and_custody_rehearsal_uses_no_synthetic_bypass() -> None:
...
def test_v5_complete_500_run_rehearsal() -> None:
    result = fresh_pilot_v5.run_synthetic_rehearsal(ROOT)
    assert result["status"] == "pass"
    assert result["tasks"] == 50
...
def test_v5_route_token_caps_fail_closed(
    tmp_path: Path,
    provider: str,
    input_tokens: int,
    output_tokens: int,
...
def test_v5_offline_module_has_no_credential_or_provider_runtime() -> None:
    source = (ROOT / "src/distributed_discovery/benchmark/agents_v1/fresh_pilot_v5.py").read_text(
        encoding="utf-8"
    )
...
def _synthetic_generic_authorization() -> dict[str, object]:
    request = fresh_pilot_v5.load_request(ROOT)
    return {
        "schema_version": "agent-ops-owner-authorization-v1",
        "kind": "owner-authorization",
        "synthetic": True,
        "gate_id": fresh_pilot_v5.GATE_ID,
        "issue": fresh_pilot_v5.ISSUE,
        "pull_request": 0,
        "branch": fresh_pilot_v5.BRANCH,
        "commit": request["starting_main"],
        "task_contract_sha256": "sha256:" + "0" * 64,
        "tree_hashes": fresh_pilot_v5.execution_tree_hashes(ROOT),
...
def test_v5_live_driver_uses_distinct_private_root() -> None:
    path = private_state_root().as_posix()
    assert path.endswith("/treasurebench-agents-v1/repair-confirmation-v5")
    assert "repair-confirmation-v1" not in path


def test_v5_live_driver_stops_before_authorization_and_credentials(
...
def test_v5_staged_mock_driver_completes_and_resumes_without_calls(tmp_path: Path) -> None:
    root = tmp_path / "fresh-v5-mock-live"
    ...
    assert first["campaign_id"] == fresh_pilot_v5.CAMPAIGN_ID
    assert first["batch_id"] == fresh_pilot_v5.BATCH_ID
    ...
    assert first["provider_operational_missing_pairings"] == 0
    assert first["provider_contract_or_safety_failure_pairings"] == 0
...
def test_v5_zero_mixed_and_all_invalid_scenarios_preserve_denominators() -> None:
    result = fresh_pilot_v5.run_protocol_validity_scenarios(ROOT)
    assert result["status"] == "pass"
    assert result["first_middle_final_invalid_tested"] is True
    assert result["protocol_invalid_alone_quarantines"] is False
    assert result["isolated_provider_operational_missing_alone_quarantines"] is False
...
def test_v5_invalid_output_separates_no_action_credit_from_feasible_bounds() -> None:
    task = fresh_pilot_v5.generate_tasks(
        ROOT,
        material="FRESH-RC-V5-INVALID-BOUND-TEST",
        public_fixture=True,
    )[0]
    classification = classify_completed_pairing(
        pairing_id="invalid-bound-test",
        model=fresh_pilot_v5.MODELS[0],
...
def _policy_runner_adapters(
    ...
    *,
    ...
    response_key = b"r" * 32
    underlying = {
        fresh_pilot_v5.MODELS[0]: (
            MockAdapter("scripted", script=("{}",))
            if first_mode == "protocol-invalid"
            else MockAdapter(first_mode)
        ),
        fresh_pilot_v5.MODELS[1]: MockAdapter(),
    }
    adapters = {
        model: fresh_pilot_v5_live._resumable(
            underlying=underlying[model],
            provider=fresh_pilot_v5.PROVIDERS[index],
            model=model,
            ledger=ledger,
            response_root=tmp_path / "responses" / fresh_pilot_v5.PROVIDERS[index].lower(),
            response_key=response_key,
        )
        for index, model in enumerate(fresh_pilot_v5.MODELS)
    }
    return adapters, underlying


def test_v5_runner_retains_protocol_invalid_pairings_without_replacement(
    tmp_path: Path,
) -> None:
    ...
    task = fresh_pilot_v5.generate_tasks(
        ROOT,
        material="FRESH-RC-V5-PROTOCOL-INVALID-TEST",
        public_fixture=True,
    )[0]
    runner = fresh_pilot_v5_live._runner(tmp_path, ledger, b"k" * 32)
    ...
    assert not tuple((tmp_path / "encrypted-terminal-classifications").glob("*.sealed"))
    ...
    assert result["provider_operational_missing"] == 0
    ...
    assert len(tuple((tmp_path / "encrypted-terminal-classifications").glob("*.sealed"))) == 10
    ...
    invalid_adapter = underlying[fresh_pilot_v5.MODELS[0]]
    assert invalid_adapter.calls == sum(
        2
        for record in ledger.records
        if record["event_type"] == "provider-call"
        and record["model"] == fresh_pilot_v5.MODELS[0]
        and record["schema_retry"] is False
    )


def test_v5_runner_provider_contract_failure_stops_before_later_calls(
    tmp_path: Path,
) -> None:
    ledger = AppendOnlyLedger(tmp_path / "ledger.jsonl")
    adapters, underlying = _policy_runner_adapters(
    ...
    task = fresh_pilot_v5.generate_tasks(
        ROOT,
        material="FRESH-RC-V5-PROVIDER-TERMINAL-TEST",
        public_fixture=True,
    )[0]
    runner = fresh_pilot_v5_live._runner(tmp_path, ledger, b"k" * 32)
    with pytest.raises(
        ProviderContractSafetyError,
        match="provider-contract-or-safety-failure",
    ):
        runner.run_stage(
            stage="private-prefix",
            tasks=(task,),
    ...
    assert underlying[fresh_pilot_v5.MODELS[0]].calls == 1
    assert underlying[fresh_pilot_v5.MODELS[1]].calls == 0
...
def test_v5_live_adapters_request_return_and_retain_only_exact_two_credentials(
    tmp_path: Path,
    ...
    monkeypatch.setattr(fresh_pilot_v5_live, "OpenAIResponsesAdapter", capture_openai)
    monkeypatch.setattr(fresh_pilot_v5_live, "AnthropicMessagesAdapter", capture_anthropic)
    ledger = RouteCappedLedger(tmp_path / "ledger.jsonl")
    _, credentials, underlying = fresh_pilot_v5_live._live_adapters(
        repo,
    ...
    fresh_pilot_v5_live._clear_stage_secrets(credentials, underlying)
...
def test_v5_credential_source_refuses_before_adapter_or_call(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    case: str,
) -> None:
    ...
    monkeypatch.setattr(fresh_pilot_v5_live, "OpenAIResponsesAdapter", prohibit_adapter)
    monkeypatch.setattr(fresh_pilot_v5_live, "AnthropicMessagesAdapter", prohibit_adapter)
    with pytest.raises(PermissionError):
        fresh_pilot_v5_live._live_adapters(
            repo,
            _synthetic_generic_authorization(),
...
def test_v5_preflight_failures_precede_credential_ingress(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    failure_point: str,
) -> None:
    ...
    monkeypatch.setattr(fresh_pilot_v5_live, "load_credentials", prohibit_credentials)
    monkeypatch.setattr(
        fresh_pilot_v5_live,
    ...
    with pytest.raises(PermissionError, match=expected):
        fresh_pilot_v5_live._prepare_live_stage(ROOT)
...
def test_v5_every_provider_stage_and_exit_path_clears_selected_secrets(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    stage: str,
    outcome: str,
    ...
    credentials = CredentialSet(
        ...
        allowed_names=fresh_pilot_v5_live.CREDENTIAL_NAMES,
        configured={name: True for name in fresh_pilot_v5_live.CREDENTIAL_NAMES},
        unused_present=(),
    )
    adapters = (
    ...
    ledger = RouteCappedLedger(tmp_path / "ledger.jsonl")
    prepared = fresh_pilot_v5_live.PreparedLiveStage(
        authorization=_synthetic_generic_authorization(),
        root=tmp_path / "state",
    ...
    monkeypatch.setattr(fresh_pilot_v5_live, "_prepare_live_stage", lambda repo: prepared)
    monkeypatch.setattr(
        fresh_pilot_v5_live,
        "_live_adapters",
    ...
    monkeypatch.setattr(fresh_pilot_v5_live, "_execute_stage", execute)
    if outcome == "success":
        assert fresh_pilot_v5_live._run_prepared_live_stage(ROOT, prepared)["status"] == "pass"
    else:
        with pytest.raises(RuntimeError, match=outcome):
            fresh_pilot_v5_live._run_prepared_live_stage(ROOT, prepared)
    assert credentials.get_secret("OPENAI_API_KEY") is None
    assert credentials.get_secret("ANTHROPIC_API_KEY") is None
...
def _prepared_stage(tmp_path: Path, stage: str) -> fresh_pilot_v5_live.PreparedLiveStage:
    return fresh_pilot_v5_live.PreparedLiveStage(
        authorization=_synthetic_generic_authorization(),
        root=tmp_path / "state",
        operational_key=b"k" * 32,
        response_root=tmp_path / "responses",
...
def test_v5_live_command_runs_all_stages_and_public_commitments_without_pause(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    stages = iter(
    ...
    monkeypatch.setattr(fresh_pilot_v5_live, "_run_prepared_live_stage", run_stage)
    monkeypatch.setattr(
        fresh_pilot_v5_live,
...
def test_v5_live_terminal_failure_quarantines_and_never_advances(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    def quarantine(
        value: fresh_pilot_v5_live.PreparedLiveStage,
        *,
        failure_class: str,
    ) -> dict[str, object]:
    ...
    monkeypatch.setattr(fresh_pilot_v5_live, "_quarantine_live_failure", quarantine)
    monkeypatch.setattr(
        fresh_pilot_v5_live,
        "_publish_public_commitment",
...
def test_v5_live_cap_guard_quarantines_before_credential_ingress(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda repo: (_ for _ in ()).throw(
            fresh_pilot_v5_live.PreparedStageFailure(
                prepared=prepared,
...
def test_v5_live_output_lock_failure_stops_for_owner(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
