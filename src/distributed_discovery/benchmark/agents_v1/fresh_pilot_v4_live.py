...
from distributed_discovery.benchmark.agents_v1.fresh_pilot_v4 import (
    BATCH_ID,
    BRANCH,
    CALLS_PER_PROVIDER,
    CAMPAIGN_ID,
...
from distributed_discovery.benchmark.agents_v1.fresh_pilot_v4_runner import (
    ProtocolValidityPilotRunner,
    ProviderTerminalMissingError,
)
from distributed_discovery.benchmark.agents_v1.fresh_pilot_v4_session import (
...
CUSTODY_REPORT = Path(
    "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v4-custody-commitment.yml"
)
LOCK_REPORT = Path(
    "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v4-output-lock-commitment.yml"
)
PRIVATE_OBJECTS = (
    "seed",
    ...
    "terminal-protocol-classifications",
    "metric-bound-ledger",
...
def private_state_root() -> Path:
    ...
    return root / "distributed-discovery" / "treasurebench-agents-v1" / "repair-confirmation-v4"
...
def _initialize_private_state(root: Path, *, repo: Path, synthetic: bool) -> Mapping[str, object]:
    ...
    _secure_directory(resolved_root / "encrypted-protocol-classifications")
    _secure_directory(resolved_root / "encrypted-metric-bounds")
    manifest: dict[str, object] = {
        "schema_version": "treasurebench-fresh-pilot-v4-private-state-v1",
        "campaign_id": CAMPAIGN_ID,
        "batch_id": BATCH_ID,
        "classification": ("synthetic-offline-only" if synthetic else "real-authorized-private"),
        "symbolic_root": (
            "XDG_STATE_HOME/distributed-discovery/treasurebench-agents-v1/repair-confirmation-v4"
        ),
        "directory_mode": "0700",
        "file_mode": "0600",
    ...
    schema = json.loads(
        (
            repo / "docs/benchmark/agents-v1/treasurebench-fresh-pilot-v4-private-state.schema.json"
...
def _load_state(path: Path) -> dict[str, object]:
    if path.exists():
        ...
        return value
    return {
        "schema_version": "treasurebench-fresh-pilot-v4-stage-state-v1",
        "campaign_id": CAMPAIGN_ID,
        "batch_id": BATCH_ID,
...
def _execution_identity(repo: Path, authorization: Mapping[str, object]) -> Mapping[str, object]:
    ...
    return {
        "schema_version": "treasurebench-fresh-pilot-v4-execution-identity-v1",
...
class _ReplayOnlyAdapter:
    def __init__(self, *, provider: str, model: str) -> None:
        self.manifest = ModelManifest(
            ...
            adapter_version="fresh-pilot-v4-replay-only-v1",
            moving_alias=False,
            live_capable=False,
        )
...
def _custody(
    ...
    *,
    ...
    if synthetic or production_rehearsal:
        namespace = (
            b"fresh-live-v4-synthetic"
            if synthetic
            else b"fresh-live-v4-production-permit-rehearsal-nonsecret"
        )
        material = {
    ...
    tasks = generate_tasks(
        repo,
        material=(
            f"fresh-live-v4-synthetic/{material['seed'].hex()}"
            if synthetic
            else (
                f"fresh-live-v4-production-permit-rehearsal/{material['seed'].hex()}"
                if production_rehearsal
                else f"fresh-private-v4/{material['seed'].hex()}"
            )
        ),
    ...
    task_sealed = _load_or_create_sealed(
        ...
        domain="fresh-real-v4-private-task-batch",
        value=task_payload,
        key=material["task_key"],
    )
    answer_sealed = _load_or_create_sealed(
        root / "answer-custody.json",
        domain="fresh-real-v4-private-answer-key",
        value=answer_payload,
        key=material["answer_key"],
    )
    manifest: Mapping[str, object] = {
        "schema_version": "treasurebench-fresh-pilot-v4-custody-manifest-v1",
        "campaign_id": CAMPAIGN_ID,
        "batch_id": BATCH_ID,
        "seed_commitment": domain_commitment(
            "treasurebench-agents-v1/fresh-pilot-v4-seed", material["seed"].hex()
        ),
        "task_plaintext_commitment": domain_commitment(
            "treasurebench-agents-v1/fresh-v4-task-batch", task_payload
        ),
        "answer_plaintext_commitment": domain_commitment(
            "treasurebench-agents-v1/fresh-v4-answer-key", answer_payload
        ),
...
def run_production_permit_custody_rehearsal(repo: Path) -> Mapping[str, object]:
    """Exercise the real v4 permit and custody ordering with disposable nonsecret data."""
    ...
    with tempfile.TemporaryDirectory(prefix="ao0008-v4-production-custody-") as temporary:
        cleanup_root = Path(temporary) / "xdg-state" / "repair-confirmation-v4"
        root = cleanup_root
        _secure_directory(root)
    ...
    return {
        "schema_version": "treasurebench-agents-v1-fresh-pilot-v4-production-custody-rehearsal-v1",
        "status": "pass",
...
def _post_lock_derived_objects(root: Path) -> dict[str, bytes]:
    ...
    for prefix, path_root in (
        ("protocol-classification", root / "encrypted-protocol-classifications"),
...
def _validate_identity_correspondence(
    ...
    actual_classification_names = {
        path.name for path in (root / "encrypted-protocol-classifications").glob("*.sealed")
    }
    ...
    return {
        ...
        "protocol_classifications": len(actual_classification_names),
...
def _validate_acceptance(
    *,
    ...
    if nonzero or false_flags:
        raise RuntimeError("fresh v4 prospective acceptance failed; quarantine entire batch")
    ...
    *,
    ...
    if summary_path.exists():
        ...
        if (
            ...
            raise PermissionError("existing v4 closeout summary identity changed")
        ...
        if existing_summary.get("post_lock_derived_objects") != len(
            ...
            raise PermissionError("existing v4 post-lock derived commitment changed")
        if _validate_identity_correspondence(root, tasks, ledger)["exact"] is not True:
            raise PermissionError("existing v4 identity correspondence changed")
    ...
    require_commitment(
        ...
        domain_commitment("treasurebench-agents-v1/fresh-v4-task-batch", unsealed_tasks),
        domain="fresh-v4-task-batch",
    )
    require_commitment(
        str(custody["answer_plaintext_commitment"]),
        domain_commitment("treasurebench-agents-v1/fresh-v4-answer-key", unsealed_answers),
        domain="fresh-v4-answer-key",
    )
    if adapters is None:
    ...
    _validate_acceptance(
        ...
        flags={
            ...
            "all_pairings_accounted": protocol_valid + protocol_invalid == 500,
        },
    )
    derived_objects = _post_lock_derived_objects(root)
    summary: dict[str, object] = {
        "schema_version": "treasurebench-fresh-pilot-v4-redacted-summary-v1",
        "status": "pass",
        ...
        "provider_terminal_missing_pairings": sum(terminal_provider_errors.values()),
        ...
        "metric_range_errors": metric_range_errors,
        "incomplete_pairings": incomplete_pairings,
        "terminal_provider_errors": sum(terminal_provider_errors.values()),
    ...
    audit_sealed = seal_object(
        domain="fresh-v4-final-private-audit-package",
        value={
            "summary": summary,
...
def _public_commitment_record(
    ...
    if result.get("status") == "quarantined":
        return (
            LOCK_REPORT,
            {
                "schema_version": "treasurebench-fresh-pilot-v4-output-lock-commitment-v1",
            ...
            "Record AO-0010 v4 quarantine output lock",
        )
    if prepared.stage == "custody":
        return (
            CUSTODY_REPORT,
            {
                "schema_version": "treasurebench-fresh-pilot-v4-custody-commitment-v1",
            ...
            "Record AO-0010 v4 custody commitment",
        )
...
def _quarantine_live_failure(
    ...
    *,
    ...
    summary: dict[str, object] = {
        "schema_version": "treasurebench-fresh-pilot-v4-quarantine-closeout-v1",
        "status": "quarantined",
        ...
        try:
            ...
            result = _quarantine_live_failure(
                ...
                failure_class=(
                    ...
                    if isinstance(error, ProviderTerminalMissingError)
...
def audit_live_corruptions(repo: Path) -> tuple[dict[str, str], ...]:
    """Reject v4 private-state, output-lock, and terminal-provider corruptions."""
    ...
    with tempfile.TemporaryDirectory(prefix="fresh-live-v4-corruptions-") as temporary:
        root = Path(temporary)
        _secure_directory(root)
        reject(
