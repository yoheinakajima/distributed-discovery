"""Deterministic AO-0006 R2 success-or-quarantine long-session control flow."""
...
class LongSessionStopRequired(RuntimeError):
    """Raised only when the R2 owner must decide a non-routine expansion or unsafe state."""
...
class LongSessionHooks(Protocol):
    """Injected task and repository operations used by the R2 coordinator."""
...
def run_long_session_control_flow(hooks: LongSessionHooks) -> Mapping[str, object]:
    """Run the one-gate R2 control flow without routine owner checkpoints."""
