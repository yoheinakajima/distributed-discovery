"""Deterministic AO-0011 v5 success-or-quarantine long-session control flow."""
...
CONTROL_STEPS = (
    ...
    "private-prefix",
    "fixed-full-batch",
    "provider-outcome-terminalization",
    "operational-circuit-breaker",
    "provider-contract-safety",
    ...
    "provider-operational-missing-outcomes-retained",
    "response-ledger-trace-classification-bound-correspondence",
    "complete-pairings",
    "method-a-b",
    "method-c",
    "independent-provider-outcome-classification",
    "metric-intervals",
    "all-pairing-contrast-bounds",
    "independent-bound-reconstruction",
    "contamination",
    "corruptions",
    "redaction",
    "retained-state-safety",
)
...
PROVIDER_CALL_STEPS = frozenset({"public-canary", "private-prefix", "fixed-full-batch"})
...
class LongSessionStopRequired(RuntimeError):
    """Raised only when the v5 owner must decide a non-routine expansion or unsafe state."""
...
class LongSessionHooks(Protocol):
    """Injected task and repository operations used by the v5 coordinator."""
...
def run_long_session_control_flow(hooks: LongSessionHooks) -> Mapping[str, object]:
    """Run the one-gate v5 control flow without routine owner checkpoints."""
