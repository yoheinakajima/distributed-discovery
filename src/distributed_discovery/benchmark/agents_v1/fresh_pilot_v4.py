"""Offline controls for the wholly fresh TreasureBench repair-confirmation v4 pilot.
...
TASK_ID = "AO-0010"
ISSUE = 208
BRANCH = "codex/treasurebench-agents-v1-fresh-pilot-v4"
GATE_ID = "AOG-AO-0010-FRESH-PILOT-V4"
...
REQUEST_PATH = Path("docs/benchmark/agents-v1/treasurebench-fresh-pilot-v4-request.yml")
ALLOCATION_PATH = Path("docs/benchmark/agents-v1/treasurebench-fresh-pilot-v4-allocation.yml")
EXECUTION_BUDGET_PATH = Path(
    "docs/benchmark/agents-v1/treasurebench-fresh-pilot-v4-execution-budget.yml"
)
CORRUPTIONS_PATH = Path("docs/benchmark/agents-v1/treasurebench-fresh-pilot-v4-corruptions.yml")
POLICY_PATH = Path("docs/benchmark/agents-v1/protocol-validity-policy-v2.yml")
GATE_PATH = Path("reports/agent-ops/AO-0010-treasurebench-fresh-pilot-v4-owner-gate.yml")
CONTRACT_PATH = Path("tasks/treasurebench-agents-v1-fresh-pilot-v4.yml")
...
def generate_authorized_private_tasks(
    ...
    *,
    ...
    return generate_tasks(
        repo,
        material=f"fresh-private-v4/{seed.hex()}",
...
def validate_registration(repo: Path) -> dict[str, object]:
    ...
    policy = load_protocol_validity_policy(repo)
    audit = load_yaml(
        repo / "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v4-provider-audit.yml"
    )
    ...
    if request["status"] not in {
        "policy-v2-frozen-execution-preparation-owner-gate-not-authorized",
        "exact-execution-frozen-owner-gate-not-authorized",
    }:
        raise ValueError("v4 request status is not authorization-free")
    ...
    if policy["action_contract"]["final_action_cardinality"] != ("exactly-one-per-required-agent"):
        raise ValueError("repaired final-action contract changed")
    if policy["status_model"]["statuses"] != [
        "batch-integrity-valid",
        "provider-terminal-missing",
        "protocol-valid",
        "protocol-invalid",
    ]:
        raise ValueError("policy v2 four-status separation changed")
    if policy["batch_acceptance"]["protocol_invalid_alone_quarantines_batch"] is not False:
        raise ValueError("protocol-invalid alone cannot quarantine v4")
    if policy["batch_acceptance"]["global_protocol_conformance_threshold"] is not None:
        raise ValueError("v4 cannot register a global protocol-conformance threshold")
    if policy["protocol_invalid_handling"]["replace_or_rerun"] is not False:
        raise ValueError("v4 protocol-invalid pairings cannot be replaced or rerun")
    if policy["protocol_invalid_handling"]["invalid_action_credit"] is not False:
        raise ValueError("v4 invalid actions cannot receive credit")
    if len(policy["metric_bounds"]["metrics"]) != 15:
        raise ValueError("policy v2 metric-bound registry changed")
    ...
    if request["credential_ingress"] != {
        ...
        raise PermissionError("fresh v4 credential ingress contract changed")
    ...
    if len(corruptions["corruptions"]) != 76:
...
def run_synthetic_rehearsal(repo: Path) -> dict[str, object]:
    ...
    task_key = hashlib.sha256(b"fresh-rc-v4/synthetic/task-key").digest()
    answer_key = hashlib.sha256(b"fresh-rc-v4/synthetic/answer-key").digest()
    task_seal = seal_object(
        domain="fresh-synthetic-v4-task-batch",
        value=[task.visible_record() for task in tasks],
        key=task_key,
        nonce=hashlib.sha256(b"fresh-rc-v4/task-nonce").digest()[:12],
        campaign_id=CAMPAIGN_ID,
        batch_id=BATCH_ID,
    )
    answers = [task.evaluator_record() for task in tasks]
    answer_seal = seal_object(
        domain="fresh-synthetic-v4-answer-key",
        value=answers,
        key=answer_key,
        nonce=hashlib.sha256(b"fresh-rc-v4/answer-nonce").digest()[:12],
        campaign_id=CAMPAIGN_ID,
        batch_id=BATCH_ID,
    )
    runs = 0
    ...
    with tempfile.TemporaryDirectory(prefix="fresh-rc-v4-rehearsal-") as temporary:
        ...
        for model_index, model in enumerate(MODELS):
            for task in tasks:
                for architecture in ARCHITECTURES:
                    ...
                    sealed = seal_object(
                        domain=f"fresh-synthetic-v4-trace/{trace_id}",
                        value=trace.raw,
                        key=hashlib.sha256(f"fresh-rc-v4/{trace_id}".encode()).digest(),
                        nonce=hashlib.sha256(f"fresh-rc-v4/nonce/{trace_id}".encode()).digest()[
                            :12
                        ],
                        campaign_id=CAMPAIGN_ID,
                        batch_id=BATCH_ID,
    ...
    primary_bounds = architecture_contrast_bounds(intervals)
    ...
    summary: dict[str, object] = {
        "schema_version": "treasurebench-agents-v1-fresh-pilot-v4-rehearsal-v1",
        "status": "pass",
        ...
        "provider_terminal_missing_pairings": 0,
...
def run_protocol_validity_scenarios(repo: Path) -> dict[str, object]:
    """Rehearse zero, mixed, and all-invalid 500-pairing policy-v2 batches."""
    ...
    if len(intended) != 500:
        raise AssertionError("policy-v2 scenario registry must contain 500 pairings")
    scenario_indices = {
    ...
    zero_classifications: tuple[PairingClassification, ...] | None = None
    for scenario, invalid_indices in scenario_indices.items():
        classifications: list[PairingClassification] = []
        intervals: list[MetricInterval] = []
        schema_repairs = 0
        constructed_or_credited_invalid_actions = 0
        for index, (model, task, architecture) in enumerate(intended):
            ...
            classification = classify_completed_pairing(
                ...
                provider_response_completed=all(
                    turn.response.error_class is None for turn in run.turns
                ),
                method_c_errors=contract.errors,
            )
            expected_status = PROTOCOL_INVALID if invalid else PROTOCOL_VALID
            ...
            if invalid:
                if run.final_actions:
                    constructed_or_credited_invalid_actions += len(run.final_actions)
                if not run.turns or any(turn.retry_count != 1 for turn in run.turns):
                    raise AssertionError("invalid pairing did not use exactly one schema repair")
                if adapter.calls != len(run.turns) * 2:
                    raise AssertionError("invalid pairing exceeded or skipped its repair bound")
        ...
        disposition = assess_batch(
            intended_pairing_ids=(
                f"{model}/{task.commitment}/{architecture}"
                for model, task, architecture in intended
            ),
            classifications=classifications,
        )
        if disposition.quarantined:
            raise AssertionError("protocol-invalid outcomes alone quarantined a scenario")
        if disposition.protocol_invalid != len(invalid_indices):
            raise AssertionError("scenario protocol-invalid count changed")
        ...
        if constructed_or_credited_invalid_actions:
            raise AssertionError("protocol-invalid output received constructed action credit")
        scenario_results[scenario] = {
            ...
            "provider_terminal_missing": disposition.provider_terminal_missing,
    ...
    if zero_classifications is None:
        raise AssertionError("zero-invalid baseline scenario was not produced")

    first = zero_classifications[0]
    provider_missing = classify_completed_pairing(
        pairing_id=first.pairing_id,
        model=first.model,
        task_commitment=first.task_commitment,
        architecture_id=first.architecture_id,
        trace_id=first.trace_id,
        provider_response_completed=False,
        method_c_errors=(),
    )
    provider_terminal_classifications = (provider_missing, *zero_classifications[1:])
    provider_terminal = assess_batch(
        intended_pairing_ids=(item.pairing_id for item in zero_classifications),
        classifications=provider_terminal_classifications,
    )
    if (
        not provider_terminal.quarantined
        or provider_terminal.provider_terminal_missing != 1
        or "provider-terminal-missing" not in provider_terminal.quarantine_reasons
    ):
        raise AssertionError("provider terminal missingness did not quarantine")
    integrity_failure = assess_batch(
        intended_pairing_ids=(item.pairing_id for item in zero_classifications),
    ...
    if (
        ...
        or "missing-or-duplicate-pairing" not in integrity_failure.quarantine_reasons
    ):
        raise AssertionError("pairing integrity failure did not quarantine")

    conditional = valid_output_conditional_diagnostic(
        (0, 1),
        label=CONDITIONAL_DIAGNOSTIC_ROLE,
    )
    try:
        valid_output_conditional_diagnostic(
            (0, 1),
    ...
    if not complete_case_overclaim_rejected:
        raise AssertionError("complete-case unconditional overclaim was accepted")
    return {
        "schema_version": "treasurebench-agents-v1-protocol-validity-v2-scenarios-v1",
        "status": "pass",
        "classification": "public-synthetic-only",
        "task_id": TASK_ID,
        "campaign_id": CAMPAIGN_ID,
        ...
        "provider_terminal_missing_quarantines": True,
...
def audit_corruptions(repo: Path) -> tuple[dict[str, str], ...]:
    ...
    reject(
        ...
        lambda: load_owner_authorization(repo, Path("/nonexistent/AOG-AO-0010-FRESH-PILOT-V4.yml")),
    )
    reject(
        "AUTH-02-synthetic",
    ...
    key = hashlib.sha256(b"fresh-rc-v4-corruption-key").digest()
    sealed = seal_object(
        domain="fresh-rc-v4-corruption",
        value={"ok": True},
        key=key,
        nonce=hashlib.sha256(b"fresh-rc-v4-corruption-nonce").digest()[:12],
        campaign_id=CAMPAIGN_ID,
        batch_id=BATCH_ID,
    )
    reject(
    ...
    reject_policy_mutation(
        ...
        ["batch-integrity-valid", "protocol-valid", "protocol-invalid"],
    )
    reject_policy_mutation(
        "POLICY-02-protocol-invalid-quarantines",
        ("batch_acceptance", "protocol_invalid_alone_quarantines_batch"),
        True,
    )
    reject_policy_mutation(
        "POLICY-03-provider-terminal-not-quarantined",
        ("batch_acceptance", "batch_quarantine_triggers"),
        [
            item
            for item in policy["batch_acceptance"]["batch_quarantine_triggers"]
            if item != "provider-terminal-missing"
        ],
    )
    reject_policy_mutation(
        "POLICY-04-silent-normalization",
        ("retry", "silent_normalization"),
        True,
    )
    reject_policy_mutation(
        "POLICY-05-invalid-action-credit",
        ("protocol_invalid_handling", "invalid_action_credit"),
        True,
    )
    reject_policy_mutation(
        "POLICY-06-replacement-pairing",
        ("retry", "replacement_pairings"),
        1,
    )
    reject_policy_mutation(
        "POLICY-07-semantic-retry",
        ("retry", "semantic_answer_retries"),
        1,
    )
    reject(
    ...
    reject_policy_mutation(
        ...
        ("batch_acceptance", "global_protocol_conformance_threshold"),
        "0.95",
    )
    reject_policy_mutation(
    ...
    reject_policy_mutation(
        ...
        ("retry", "transport", "maximum_attempts"),
        3,
    )
    reject_policy_mutation(
        "RETRY-02-second-schema-repair",
        ("retry", "schema", "maximum_repairs"),
        2,
    )
    reject_policy_mutation(
    ...
    reject_policy_mutation(
        ...
        ("retry", "outcome_dependent_retries"),
        1,
    )
...
def _validate_policy_semantics(value: Mapping[str, object]) -> None:
    groups = (
        value["status_model"],
        value["retry"],
        value["protocol_invalid_handling"],
        value["batch_acceptance"],
        value["reporting"],
    )
    if not all(isinstance(item, Mapping) for item in groups):
        raise TypeError("policy-v2 control groups must be mappings")
    ...
    acceptance = cast(Mapping[str, object], groups[3])
    reporting = cast(Mapping[str, object], groups[4])
    if status_model["statuses"] != [
        "batch-integrity-valid",
        "provider-terminal-missing",
        "protocol-valid",
        "protocol-invalid",
    ]:
        raise ValueError("policy-v2 status separation changed")
    ...
    if not isinstance(raw_transport, Mapping) or not isinstance(raw_schema_retry, Mapping):
        raise TypeError("policy-v2 retry controls must be mappings")
    ...
    if transport["maximum_attempts"] != 2:
        raise ValueError("policy-v2 transport attempts changed")
    if schema_retry["maximum_repairs"] != 1:
        raise ValueError("policy-v2 schema repair bound changed")
    if (
        ...
        raise ValueError("policy-v2 retry or normalization controls changed")
    if handling["invalid_action_credit"] is not False:
        raise ValueError("policy-v2 invalid-output credit changed")
    if acceptance["protocol_invalid_alone_quarantines_batch"] is not False:
        raise ValueError("protocol-invalid alone cannot quarantine")
    if acceptance["global_protocol_conformance_threshold"] is not None:
        raise ValueError("policy-v2 cannot register a conformance threshold")
    triggers = acceptance["batch_quarantine_triggers"]
    if (
        not isinstance(triggers, Sequence)
        or "provider-terminal-missing" not in triggers
        or "missing-or-duplicate-pairing" not in triggers
    ):
        raise ValueError("policy-v2 quarantine triggers changed")
    if (
        reporting["composite_score"] is not None
        or reporting["provider_or_model_ranking"] is not False
        or reporting["leaderboard"] is not False
    ):
        raise ValueError("policy-v2 ranking or composite prohibition changed")
