"""Deterministic AO-0008 v3 success-or-quarantine long-session control flow."""
...
class LongSessionStopRequired(RuntimeError):
    """Raised only when the v3 owner must decide a non-routine expansion or unsafe state."""
...
class LongSessionHooks(Protocol):
    """Injected task and repository operations used by the v3 coordinator."""
...
def run_long_session_control_flow(hooks: LongSessionHooks) -> Mapping[str, object]:
    """Run the one-gate v3 control flow without routine owner checkpoints."""
