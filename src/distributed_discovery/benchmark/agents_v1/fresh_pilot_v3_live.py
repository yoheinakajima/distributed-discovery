...
CUSTODY_REPORT = Path(
    "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v3-custody-commitment.yml"
)
LOCK_REPORT = Path(
    "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v3-output-lock-commitment.yml"
)
PRIVATE_OBJECTS = (
    "seed",
...
def _initialize_private_state(root: Path, *, repo: Path, synthetic: bool) -> Mapping[str, object]:
    ...
    manifest: dict[str, object] = {
        "schema_version": "treasurebench-fresh-pilot-v3-private-state-v1",
        "campaign_id": CAMPAIGN_ID,
        "batch_id": BATCH_ID,
    ...
    schema = json.loads(
        (
            repo / "docs/benchmark/agents-v1/treasurebench-fresh-pilot-v3-private-state.schema.json"
...
def _load_state(path: Path) -> dict[str, object]:
    if path.exists():
        ...
        return value
    return {
        "schema_version": "treasurebench-fresh-pilot-v3-stage-state-v1",
        "campaign_id": CAMPAIGN_ID,
        "batch_id": BATCH_ID,
...
def _execution_identity(repo: Path, authorization: Mapping[str, object]) -> Mapping[str, object]:
    ...
    return {
        "schema_version": "treasurebench-fresh-pilot-v3-execution-identity-v1",
...
class _ReplayOnlyAdapter:
    def __init__(self, *, provider: str, model: str) -> None:
        self.manifest = ModelManifest(
            ...
            adapter_version="fresh-pilot-v3-replay-only-v1",
            moving_alias=False,
            live_capable=False,
        )
    ...
    *,
    ...
    if synthetic or production_rehearsal:
        namespace = (
            ...
            if synthetic
            else b"fresh-live-v3-production-permit-rehearsal-nonsecret"
        )
        material = {
    ...
    tasks = generate_tasks(
        repo,
        material=(
            f"fresh-live-v3-synthetic/{material['seed'].hex()}"
            if synthetic
            else (
                f"fresh-live-v3-production-permit-rehearsal/{material['seed'].hex()}"
                if production_rehearsal
                else f"fresh-private-v3/{material['seed'].hex()}"
            )
        ),
    ...
    task_sealed = _load_or_create_sealed(
        ...
        domain="fresh-real-v3-private-task-batch",
        value=task_payload,
        key=material["task_key"],
    )
    answer_sealed = _load_or_create_sealed(
        root / "answer-custody.json",
        domain="fresh-real-v3-private-answer-key",
        value=answer_payload,
        key=material["answer_key"],
    )
    manifest: Mapping[str, object] = {
        "schema_version": "treasurebench-fresh-pilot-v3-custody-manifest-v1",
        "campaign_id": CAMPAIGN_ID,
        "batch_id": BATCH_ID,
        "seed_commitment": domain_commitment(
            "treasurebench-agents-v1/fresh-pilot-v3-seed", material["seed"].hex()
        ),
        "task_plaintext_commitment": domain_commitment(
            "treasurebench-agents-v1/fresh-v3-task-batch", task_payload
        ),
        "answer_plaintext_commitment": domain_commitment(
            "treasurebench-agents-v1/fresh-v3-answer-key", answer_payload
        ),
...
def run_production_permit_custody_rehearsal(repo: Path) -> Mapping[str, object]:
    """Exercise the real v3 permit and custody ordering with disposable nonsecret data."""
    ...
    with tempfile.TemporaryDirectory(prefix="ao0008-v3-production-custody-") as temporary:
        cleanup_root = Path(temporary) / "xdg-state" / "repair-confirmation-v3"
        root = cleanup_root
        _secure_directory(root)
    ...
    return {
        "schema_version": "treasurebench-agents-v1-fresh-pilot-v3-production-custody-rehearsal-v1",
        "status": "pass",
...
def _validate_acceptance(
    *,
    ...
    if nonzero or false_flags:
        raise RuntimeError("fresh v3 prospective acceptance failed; quarantine entire batch")
...
def _execute_stage(
    ...
    *,
    ...
    require_commitment(
        ...
        domain_commitment("treasurebench-agents-v1/fresh-v3-task-batch", unsealed_tasks),
        domain="fresh-v3-task-batch",
    )
    require_commitment(
        str(custody["answer_plaintext_commitment"]),
        domain_commitment("treasurebench-agents-v1/fresh-v3-answer-key", unsealed_answers),
        domain="fresh-v3-answer-key",
    )
    if adapters is None:
    ...
    summary: dict[str, object] = {
        "schema_version": "treasurebench-fresh-pilot-v3-redacted-summary-v1",
        "status": "pass",
    ...
    audit_sealed = seal_object(
        domain="fresh-v3-final-private-audit-package",
        value={
            "summary": summary,
...
def _public_commitment_record(
    ...
    if result.get("status") == "quarantined":
        return (
            LOCK_REPORT,
            {
                "schema_version": "treasurebench-fresh-pilot-v3-output-lock-commitment-v1",
            ...
            "Record AO-0008 v3 quarantine output lock",
        )
    if prepared.stage == "custody":
        return (
            CUSTODY_REPORT,
            {
                "schema_version": "treasurebench-fresh-pilot-v3-custody-commitment-v1",
            ...
            "Record AO-0008 v3 custody commitment",
        )
...
def _quarantine_live_failure(
    ...
    *,
    ...
    summary: dict[str, object] = {
        "schema_version": "treasurebench-fresh-pilot-v3-quarantine-closeout-v1",
        "status": "quarantined",
...
def audit_live_corruptions(repo: Path) -> tuple[dict[str, str], ...]:
    """Reject v3 private-state, output-lock, and terminal-provider corruptions."""
    ...
    with tempfile.TemporaryDirectory(prefix="fresh-live-v3-corruptions-") as temporary:
        root = Path(temporary)
        _secure_directory(root)
        reject(
