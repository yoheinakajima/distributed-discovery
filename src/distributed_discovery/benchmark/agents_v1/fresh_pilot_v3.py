"""Offline controls for the wholly fresh TreasureBench repair-confirmation v3 pilot.
...
TASK_ID = "AO-0008"
ISSUE = 204
BRANCH = "codex/treasurebench-agents-v1-fresh-pilot-v3"
GATE_ID = "AOG-AO-0008-FRESH-PILOT-V3"
...
REQUEST_PATH = Path("docs/benchmark/agents-v1/treasurebench-fresh-pilot-v3-request.yml")
ALLOCATION_PATH = Path("docs/benchmark/agents-v1/treasurebench-fresh-pilot-v3-allocation.yml")
EXECUTION_BUDGET_PATH = Path(
    "docs/benchmark/agents-v1/treasurebench-fresh-pilot-v3-execution-budget.yml"
)
CORRUPTIONS_PATH = Path("docs/benchmark/agents-v1/treasurebench-fresh-pilot-v3-corruptions.yml")
GATE_PATH = Path("reports/agent-ops/AO-0008-treasurebench-fresh-pilot-v3-owner-gate.yml")
CONTRACT_PATH = Path("tasks/treasurebench-agents-v1-fresh-pilot-v3.yml")
...
def generate_authorized_private_tasks(
    ...
    *,
    ...
    return generate_tasks(
        repo,
        material=f"fresh-private-v3/{seed.hex()}",
...
def validate_registration(repo: Path) -> dict[str, object]:
    ...
    audit = load_yaml(
        repo / "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v3-provider-audit.yml"
    )
    ...
    if request["credential_ingress"] != {
        ...
        raise PermissionError("fresh v3 credential ingress contract changed")
    ...
    if len(corruptions["corruptions"]) != 43:
...
def run_synthetic_rehearsal(repo: Path) -> dict[str, object]:
    ...
    task_key = hashlib.sha256(b"fresh-rc-v3/synthetic/task-key").digest()
    answer_key = hashlib.sha256(b"fresh-rc-v3/synthetic/answer-key").digest()
    task_seal = seal_object(
        domain="fresh-synthetic-v3-task-batch",
        value=[task.visible_record() for task in tasks],
        key=task_key,
        nonce=hashlib.sha256(b"fresh-rc-v3/task-nonce").digest()[:12],
        campaign_id=CAMPAIGN_ID,
        batch_id=BATCH_ID,
    )
    answers = [task.evaluator_record() for task in tasks]
    answer_seal = seal_object(
        domain="fresh-synthetic-v3-answer-key",
        value=answers,
        key=answer_key,
        nonce=hashlib.sha256(b"fresh-rc-v3/answer-nonce").digest()[:12],
        campaign_id=CAMPAIGN_ID,
        batch_id=BATCH_ID,
    )
    runs = 0
    ...
    with tempfile.TemporaryDirectory(prefix="fresh-rc-v3-rehearsal-") as temporary:
        ...
        for model_index, model in enumerate(MODELS):
            for task in tasks:
                for architecture in ARCHITECTURES:
                    ...
                    sealed = seal_object(
                        domain=f"fresh-synthetic-v3-trace/{trace_id}",
                        value=trace.raw,
                        key=hashlib.sha256(f"fresh-rc-v3/{trace_id}".encode()).digest(),
                        nonce=hashlib.sha256(f"fresh-rc-v3/nonce/{trace_id}".encode()).digest()[
                            :12
                        ],
                        campaign_id=CAMPAIGN_ID,
                        batch_id=BATCH_ID,
    ...
    summary: dict[str, object] = {
        "schema_version": "treasurebench-agents-v1-fresh-pilot-v3-rehearsal-v1",
        "status": "pass",
...
def audit_corruptions(repo: Path) -> tuple[dict[str, str], ...]:
    ...
    reject(
        ...
        lambda: load_owner_authorization(repo, Path("/nonexistent/AOG-AO-0008-FRESH-PILOT-V3.yml")),
    )
    reject(
        "AUTH-02-synthetic",
    ...
    key = hashlib.sha256(b"fresh-rc-v3-corruption-key").digest()
    sealed = seal_object(
        domain="fresh-rc-v3-corruption",
        value={"ok": True},
        key=key,
        nonce=hashlib.sha256(b"fresh-rc-v3-corruption-nonce").digest()[:12],
        campaign_id=CAMPAIGN_ID,
        batch_id=BATCH_ID,
    )
    reject(
