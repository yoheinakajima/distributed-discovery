...
CUSTODY_REPORT = Path(
    "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v5-custody-commitment.yml"
)
LOCK_REPORT = Path(
    "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v5-output-lock-commitment.yml"
)
PRIVATE_OBJECTS = (
    "seed",
    ...
    "terminal-pairing-classifications",
    "metric-bound-ledger",
...
def private_state_root() -> Path:
    ...
    return root / "distributed-discovery" / "treasurebench-agents-v1" / "repair-confirmation-v5"
...
def _initialize_private_state(root: Path, *, repo: Path, synthetic: bool) -> Mapping[str, object]:
    ...
    _secure_directory(resolved_root / "encrypted-terminal-classifications")
    _secure_directory(resolved_root / "encrypted-metric-bounds")
    _secure_directory(resolved_root / "encrypted-provider-outcomes")
    manifest: dict[str, object] = {
        "schema_version": "treasurebench-fresh-pilot-v5-private-state-v1",
        "campaign_id": CAMPAIGN_ID,
        "batch_id": BATCH_ID,
        "classification": ("synthetic-offline-only" if synthetic else "real-authorized-private"),
        "symbolic_root": (
            "XDG_STATE_HOME/distributed-discovery/treasurebench-agents-v1/repair-confirmation-v5"
        ),
        "directory_mode": "0700",
        "file_mode": "0600",
    ...
    schema = json.loads(
        (
            repo / "docs/benchmark/agents-v1/treasurebench-fresh-pilot-v5-private-state.schema.json"
...
def _load_state(path: Path) -> dict[str, object]:
    if path.exists():
        ...
        return value
    return {
        "schema_version": "treasurebench-fresh-pilot-v5-stage-state-v1",
        "campaign_id": CAMPAIGN_ID,
        "batch_id": BATCH_ID,
...
def _execution_identity(repo: Path, authorization: Mapping[str, object]) -> Mapping[str, object]:
    ...
    return {
        "schema_version": "treasurebench-fresh-pilot-v5-execution-identity-v1",
...
class _ReplayOnlyAdapter:
    def __init__(self, *, provider: str, model: str) -> None:
        self.manifest = ModelManifest(
            ...
            adapter_version="fresh-pilot-v5-replay-only-v1",
            moving_alias=False,
            live_capable=False,
        )
...
def _custody(
    ...
    *,
    ...
    if synthetic or production_rehearsal:
        namespace = (
            b"fresh-live-v5-synthetic"
            if synthetic
            else b"fresh-live-v5-production-permit-rehearsal-nonsecret"
        )
        material = {
            "seed": hashlib.sha256(namespace + b"/seed").digest(),
            "task_key": hashlib.sha256(namespace + b"/task-key").digest(),
            "answer_key": hashlib.sha256(namespace + b"/answer-key").digest(),
        }
    else:
    ...
    tasks = generate_tasks(
        repo,
        material=(
            f"fresh-live-v5-synthetic/{material['seed'].hex()}"
            if synthetic
            else (
                f"fresh-live-v5-production-permit-rehearsal/{material['seed'].hex()}"
                if production_rehearsal
                else f"fresh-private-v5/{material['seed'].hex()}"
            )
        ),
    ...
    task_sealed = _load_or_create_sealed(
        ...
        domain="fresh-real-v5-private-task-batch",
        value=task_payload,
        key=material["task_key"],
    )
    answer_sealed = _load_or_create_sealed(
        root / "answer-custody.json",
        domain="fresh-real-v5-private-answer-key",
        value=answer_payload,
        key=material["answer_key"],
    )
    manifest: Mapping[str, object] = {
        "schema_version": "treasurebench-fresh-pilot-v5-custody-manifest-v1",
        "campaign_id": CAMPAIGN_ID,
        "batch_id": BATCH_ID,
        "seed_commitment": domain_commitment(
            "treasurebench-agents-v1/fresh-pilot-v5-seed", material["seed"].hex()
        ),
        "task_plaintext_commitment": domain_commitment(
            "treasurebench-agents-v1/fresh-v5-task-batch", task_payload
        ),
        "answer_plaintext_commitment": domain_commitment(
            "treasurebench-agents-v1/fresh-v5-answer-key", answer_payload
        ),
...
def run_production_permit_custody_rehearsal(repo: Path) -> Mapping[str, object]:
    """Exercise the real v5 permit and custody ordering with disposable nonsecret data."""
    ...
    def rejected(name: str, action: object) -> None:
        try:
            ...
            action()
        except (FileExistsError, InvalidTag, PermissionError, ValueError):
            negative_checks.append(name)
            return
        raise AssertionError(f"production custody negative check accepted: {name}")

    with tempfile.TemporaryDirectory(prefix="ao0008-v5-production-custody-") as temporary:
        cleanup_root = Path(temporary) / "xdg-state" / "repair-confirmation-v5"
        root = cleanup_root
        _secure_directory(root)
        _secure_directory(root / "encrypted-traces")
        _initialize_private_state(root, repo=repo, synthetic=True)
        authorization = {
            "synthetic": False,
            "ao0008_production_rehearsal": True,
            "campaign_id": CAMPAIGN_ID,
        }
        tasks, material, task_sealed, answer_sealed, manifest = _custody(
            repo,
            root,
            authorization,
            synthetic=False,
        ...
        sequence.extend(
            (
                "production-private-generation-permit",
                "task-and-answer-generation",
                "independent-aes-256-gcm-encryption",
                "atomic-exclusive-persistence",
                "custody-manifest-creation",
            )
        )
        if len(tasks) != 50:
            raise AssertionError("production permit rehearsal did not generate 50 tasks")
        task_payload = [task.visible_record() for task in tasks]
        answer_payload = [task.evaluator_record() for task in tasks]
        reloaded_task = _sealed_from_record(_read_json(root / "task-custody.json"))
        reloaded_answer = _sealed_from_record(_read_json(root / "answer-custody.json"))
        associated_task = canonical_json(
            {
                "campaign_id": CAMPAIGN_ID,
                "batch_id": BATCH_ID,
                "domain": task_sealed.domain,
            }
        )
        associated_answer = canonical_json(
            {
                "campaign_id": CAMPAIGN_ID,
                "batch_id": BATCH_ID,
                "domain": answer_sealed.domain,
            }
        )
        independent_task = AESGCM(material["task_key"]).decrypt(
            bytes.fromhex(reloaded_task.nonce_hex),
            reloaded_task.ciphertext,
            associated_task,
        )
        independent_answer = AESGCM(material["answer_key"]).decrypt(
            bytes.fromhex(reloaded_answer.nonce_hex),
            reloaded_answer.ciphertext,
            associated_answer,
        )
        if canonical_json(json.loads(independent_task)) != canonical_json(task_payload):
            raise AssertionError("independent task ciphertext verification failed")
        if canonical_json(json.loads(independent_answer)) != canonical_json(answer_payload):
            raise AssertionError("independent answer ciphertext verification failed")
        if _read_json(root / "custody-manifest.json") != manifest:
            raise AssertionError("reloaded custody manifest differs")
        for path in (
            root / "task-custody.json",
            root / "answer-custody.json",
            ...
            if stat.S_IMODE(path.stat().st_mode) != 0o600:
                raise PermissionError("production custody rehearsal file mode changed")
        sequence.append("reload-and-exact-independent-verification")

        rejected(
            "wrong-task-key",
            lambda: AESGCM(b"x" * 32).decrypt(
                bytes.fromhex(reloaded_task.nonce_hex),
                reloaded_task.ciphertext,
                associated_task,
            ),
        ...
        rejected(
            "corrupted-task-ciphertext",
            lambda: AESGCM(material["task_key"]).decrypt(
                bytes.fromhex(reloaded_task.nonce_hex),
                reloaded_task.ciphertext[:-1] + bytes([reloaded_task.ciphertext[-1] ^ 1]),
                associated_task,
            ),
        )
        rejected(
            "wrong-campaign-associated-data",
        ...
        rejected(
            "duplicate-exclusive-persistence",
            lambda: _create_json(root / "custody-manifest.json", manifest),
        )
        unsafe_link = root.parent / "unsafe-link"
        unsafe_link.symlink_to(root, target_is_directory=True)
        rejected("symlink-state-root", lambda: _secure_directory(unsafe_link))
        if len(negative_checks) != 6:
            raise AssertionError("production custody rehearsal negative-check count changed")

    if cleanup_root is None or cleanup_root.exists():
        raise AssertionError("production custody rehearsal did not clean temporary state")
    sequence.append("deterministic-cleanup")
    return {
        "schema_version": "treasurebench-agents-v1-fresh-pilot-v5-production-custody-rehearsal-v1",
        "status": "pass",
        "classification": "authorization-free-disposable-nonsecret-production-path",
        "task_id": "AO-0011",
        "campaign_id": CAMPAIGN_ID,
        "batch_id": BATCH_ID,
        "production_generator_permit_path": "pass",
        "synthetic_mode_bypass_used": False,
        "tasks": 50,
        "answers": 50,
        ...
        "ordering": sequence,
        "atomic_exclusive_persistence": "pass",
        "file_modes": "0600",
        "directory_modes": "0700",
        "cleanup": "pass",
        "real_seed_or_private_campaign_material_created": False,
        "credentials_read": 0,
        "provider_calls": 0,
        "spend_usd": "0",
    }
...
def _output_objects(root: Path, ledger: AppendOnlyLedger) -> dict[str, bytes]:
    """Return the immutable provider-output inventory fixed before unsealing."""
    ...
    for path in sorted((root / "encrypted-provider-outcomes").glob("*.sealed")):
        objects[f"provider-outcome/{path.name}"] = path.read_bytes()
    response_root = root / "encrypted-provider-responses"
...
def _post_lock_derived_objects(root: Path) -> dict[str, bytes]:
    """Collect safe post-lock classifications and bounds without changing the output lock."""

    objects: dict[str, bytes] = {}
    for prefix, path_root in (
        ("terminal-classification", root / "encrypted-terminal-classifications"),
        ("metric-bound", root / "encrypted-metric-bounds"),
    ):
        for path in sorted(path_root.glob("*.sealed")):
            info = path.lstat()
            if stat.S_ISLNK(info.st_mode) or not stat.S_ISREG(info.st_mode):
                raise LongSessionStopRequired("unsafe post-lock derived object")
            objects[f"{prefix}/{path.name}"] = path.read_bytes()
    return objects


def _object_inventory_commitment(objects: Mapping[str, bytes]) -> str:
    inventory = [
        {
            "name": name,
            "sha256": f"sha256:{sha256_hex(payload)}",
        }
        for name, payload in sorted(objects.items())
    ]
    return f"sha256:{sha256_hex(canonical_json(inventory))}"
...
def _validate_identity_correspondence(
    ...
    """Require exact response-ledger and pairing-trace/classification inventories."""

    prefix_task_ids = {tasks[index].task_id for index in PREFIX_INDICES}
    expected_pairing_ids: set[str] = set()
    expected_trace_names = {
        f"{sha256_hex(f'public-canary/{model}'.encode())}.sealed" for model in MODELS
    }
    expected_classification_names: set[str] = set()
    for model in MODELS:
        for task in tasks:
            stage = "private-prefix" if task.task_id in prefix_task_ids else "fixed-full-batch"
            for architecture in ARCHITECTURES:
                pairing_id = f"{model}/{task.commitment}/{architecture}"
                trace_id = f"{stage}/{model}/{task.task_id}/{architecture}"
                expected_pairing_ids.add(pairing_id)
                expected_trace_names.add(f"{sha256_hex(trace_id.encode())}.sealed")
                expected_classification_names.add(f"{sha256_hex(pairing_id.encode())}.sealed")
    if len(expected_pairing_ids) != 500:
        raise RuntimeError("fresh intended pairing identity set is not 500")
    actual_trace_names = {path.name for path in (root / "encrypted-traces").glob("*.sealed")}
    actual_classification_names = {
        path.name for path in (root / "encrypted-terminal-classifications").glob("*.sealed")
    }
    actual_bound_names = {path.name for path in (root / "encrypted-metric-bounds").glob("*.sealed")}
    actual_outcome_names = {
        path.name for path in (root / "encrypted-provider-outcomes").glob("*.sealed")
    }
    if actual_trace_names != expected_trace_names:
        raise RuntimeError("fresh trace inventory identity correspondence failed")
    if actual_classification_names != expected_classification_names:
        raise RuntimeError("fresh classification inventory identity correspondence failed")
    if actual_bound_names != expected_classification_names:
        raise RuntimeError("fresh metric-bound inventory identity correspondence failed")
    if actual_outcome_names != expected_classification_names:
        raise RuntimeError("fresh provider-outcome inventory identity correspondence failed")

    call_records = [
        record for record in ledger.records if record.get("event_type") == "provider-call"
    ]
    expected_responses = {
        (
            str(record["provider"]).lower(),
            f"{record['call_key']}-attempt-{int(str(record['transport_attempt']))}.sealed.json",
        )
        for record in call_records
    }
    if len(expected_responses) != len(call_records):
        raise RuntimeError("fresh ledger contains duplicate provider attempt identity")
    response_root = root / "encrypted-provider-responses"
    actual_responses = {
        (path.parent.name, path.name) for path in response_root.glob("*/*.sealed.json")
    }
    if actual_responses != expected_responses:
        raise RuntimeError("fresh response-ledger identity correspondence failed")
    return {
        "intended_pairings": len(expected_pairing_ids),
        "pairing_traces": len(expected_pairing_ids),
        "public_canary_traces": len(MODELS),
        "terminal_classifications": len(actual_classification_names),
        "provider_outcome_records": len(actual_outcome_names),
        "metric_bound_records": len(actual_bound_names),
        "provider_attempt_records": len(call_records),
        "encrypted_provider_responses": len(actual_responses),
        "exact": True,
    }
...
def _validate_acceptance(
    *,
    ...
    if nonzero or false_flags:
        raise RuntimeError("fresh v5 prospective acceptance failed; quarantine entire batch")


def _combine_private_contrast_bounds(
    parts: Sequence[Sequence[Mapping[str, object]]],
) -> tuple[dict[str, object], ...]:
    """Combine disjoint prefix/full bound records by their intended denominators."""

    grouped: dict[tuple[str, str, str, str], tuple[int, Fraction, Fraction]] = {}
    for records in parts:
        for record in records:
            key = (
                str(record["model"]),
                str(record["metric_id"]),
                str(record["left_architecture"]),
                str(record["right_architecture"]),
            )
            count = int(str(record["intended_eligible_pairs"]))
            lower_total = Fraction(str(record["lower"])) * count
            upper_total = Fraction(str(record["upper"])) * count
            prior_count, prior_lower, prior_upper = grouped.get(
                key,
                (0, Fraction(0), Fraction(0)),
            )
            grouped[key] = (
                prior_count + count,
                prior_lower + lower_total,
                prior_upper + upper_total,
            )
    output: list[dict[str, object]] = []
    for key, (count, lower_total, upper_total) in sorted(grouped.items()):
        if count <= 0:
            raise ValueError("combined bound denominator must be positive")
        output.append(
            {
                "model": key[0],
                "metric_id": key[1],
                "left_architecture": key[2],
                "right_architecture": key[3],
                "intended_eligible_pairs": count,
                "lower": str(lower_total / count),
                "upper": str(upper_total / count),
            }
        )
    return tuple(output)
...
def _runner(
    ...
    return ProtocolValidityPilotRunner(
        state_root=root,
        ledger=ledger,
...
def _execute_stage(
    ...
    *,
    ...
    verify_output_lock(lock, objects, ledger=ledger)
    summary_path = root / "redacted-summary.json"
    if summary_path.exists():
        existing_summary = _read_json(summary_path)
        validate_public_pilot_summary(existing_summary)
        if (
            existing_summary.get("campaign_id") != CAMPAIGN_ID
            or existing_summary.get("batch_id") != BATCH_ID
            or existing_summary.get("output_lock_hash") != lock["lock_hash"]
        ):
            raise PermissionError("existing v5 closeout summary identity changed")
        derived_objects = _post_lock_derived_objects(root)
        if existing_summary.get("post_lock_derived_objects") != len(
            derived_objects
        ) or existing_summary.get(
            "post_lock_derived_state_commitment"
        ) != _object_inventory_commitment(derived_objects):
            raise PermissionError("existing v5 post-lock derived commitment changed")
        if _validate_identity_correspondence(root, tasks, ledger)["exact"] is not True:
            raise PermissionError("existing v5 identity correspondence changed")
        return existing_summary
    provider_calls_at_lock = sum(
        record.get("event_type") == "provider-call" for record in ledger.records
    )
    ...
    require_commitment(
        ...
        domain_commitment("treasurebench-agents-v1/fresh-v5-task-batch", unsealed_tasks),
        domain="fresh-v5-task-batch",
    )
    require_commitment(
        str(custody["answer_plaintext_commitment"]),
        domain_commitment("treasurebench-agents-v1/fresh-v5-answer-key", unsealed_answers),
        domain="fresh-v5-answer-key",
    )
    if adapters is None:
    ...
    provider_calls_after_analysis = sum(
        record.get("event_type") == "provider-call" for record in ledger.records
    )
    no_call_after_lock = provider_calls_after_analysis == provider_calls_at_lock
    verify_output_lock(lock, _output_objects(root, ledger), ledger=ledger)
    ...
    protocol_valid = int(str(prefix_verification["protocol_valid"])) + int(
        str(full_verification["protocol_valid"])
    )
    protocol_invalid = int(str(prefix_verification["protocol_invalid"])) + int(
        str(full_verification["protocol_invalid"])
    )
    provider_operational_missing = int(
        str(prefix_verification["provider_operational_missing"])
    ) + int(str(full_verification["provider_operational_missing"]))
    provider_contract_or_safety_failure = int(
        str(prefix_verification["provider_contract_or_safety_failure"])
    ) + int(str(full_verification["provider_contract_or_safety_failure"]))
    terminal_pairings_classified = int(
        str(prefix_verification["terminal_pairings_classified"])
    ) + int(str(full_verification["terminal_pairings_classified"]))
    method_c_classification_failures = int(
        str(prefix_verification["method_c_classification_failures"])
    ) + int(str(full_verification["method_c_classification_failures"]))
    metric_bound_disagreements = int(str(prefix_verification["metric_bound_disagreements"])) + int(
        str(full_verification["metric_bound_disagreements"])
    )
    stage_bound_agreement = bool(prefix_verification["independent_bound_agreement"]) and bool(
        full_verification["independent_bound_agreement"]
    )
    combined_primary_bounds = _combine_private_contrast_bounds(
        (
            cast(
                Sequence[Mapping[str, object]],
                prefix_verification["_private_primary_contrast_bounds"],
            ),
            cast(
                Sequence[Mapping[str, object]],
                full_verification["_private_primary_contrast_bounds"],
            ),
        )
    )
    combined_independent_bounds = _combine_private_contrast_bounds(
        (
            cast(
                Sequence[Mapping[str, object]],
                prefix_verification["_private_independent_contrast_bounds"],
            ),
            cast(
                Sequence[Mapping[str, object]],
                full_verification["_private_independent_contrast_bounds"],
            ),
        )
    )
    require_bound_agreement(combined_primary_bounds, combined_independent_bounds)
    independent_bound_agreement = stage_bound_agreement and bool(combined_primary_bounds)
    protocol_invalid_nonquarantine = (
        prefix_verification["protocol_invalid_alone_quarantines"] is False
        and full_verification["protocol_invalid_alone_quarantines"] is False
    )
    operational_missing_nonquarantine = (
        prefix_verification["isolated_provider_operational_missing_alone_quarantines"] is False
        and full_verification["isolated_provider_operational_missing_alone_quarantines"] is False
    )
    final_circuit = full_verification["circuit_breaker"]
    if not isinstance(final_circuit, Mapping):
        raise ValueError("fresh circuit-breaker summary is malformed")
    ...
    correspondence = _validate_identity_correspondence(root, tasks, ledger)
    ...
    _validate_acceptance(
        counters={
            "provider_contract_or_safety_failures": (provider_contract_or_safety_failure),
            "incomplete_pairings": incomplete_pairings,
            "method_a_b_disagreements": method_disagreements,
            "method_c_classification_failures": method_c_classification_failures,
            "metric_bound_disagreements": metric_bound_disagreements,
            "contamination_findings": contamination_findings,
            "metric_range_errors": metric_range_errors,
            "unclassified_pairings": 500 - terminal_pairings_classified,
        },
        flags={
            ...
            "no_call_after_lock": no_call_after_lock,
            "unseal_after_lock_verified": True,
            "identity_correspondence_exact": correspondence["exact"] is True,
            "independent_bound_agreement": independent_bound_agreement,
            "protocol_invalid_nonquarantine": protocol_invalid_nonquarantine,
            "operational_missing_nonquarantine": operational_missing_nonquarantine,
            "operational_circuit_breaker_not_fired": (final_circuit.get("fired") is False),
            "all_pairings_accounted": (
                protocol_valid + protocol_invalid + provider_operational_missing == 500
            ),
        },
    )
    derived_objects = _post_lock_derived_objects(root)
    summary: dict[str, object] = {
        "schema_version": "treasurebench-fresh-pilot-v5-redacted-summary-v1",
        "status": "pass",
        ...
        "metric_bound_disagreements": metric_bound_disagreements,
        "contamination_findings": contamination_findings,
        "protocol_valid_pairings": protocol_valid,
        "protocol_invalid_pairings": protocol_invalid,
        "provider_operational_missing_pairings": provider_operational_missing,
        "provider_contract_or_safety_failure_pairings": (provider_contract_or_safety_failure),
        "terminal_pairings_classified": terminal_pairings_classified,
        "protocol_invalid_alone_quarantined": False,
        "isolated_provider_operational_missing_alone_quarantined": False,
        "operational_circuit_breaker": dict(final_circuit),
        "invalid_final_action_cardinalities": invalid_final_cardinalities,
        "metric_range_errors": metric_range_errors,
        "incomplete_pairings": incomplete_pairings,
        "terminal_provider_operational_errors": sum(terminal_provider_errors.values()),
        "recovered_transport_warnings": sum(recovered_transport_warnings.values()),
        "usage": {
            "calls": totals["calls"],
            "input_tokens": totals["input_tokens"],
            "output_tokens": totals["output_tokens"],
        ...
        "identity_correspondence": dict(correspondence),
        "independent_metric_bound_agreement": independent_bound_agreement,
        "post_lock_derived_objects": len(derived_objects),
        "post_lock_derived_state_commitment": _object_inventory_commitment(derived_objects),
        "exact_cost_reconciliation": exact_cost_reconciliation,
        "retention_days": 365,
        "private_state_verified": private_state_verified,
        "provider_phase_closed": True,
    ...
    audit_sealed = seal_object(
        domain="fresh-v5-final-private-audit-package",
        value={
            "summary": summary,
...
def _public_commitment_record(
    ...
    if result.get("status") == "quarantined":
        return (
            LOCK_REPORT,
            {
                "schema_version": "treasurebench-fresh-pilot-v5-output-lock-commitment-v1",
            ...
            "Record AO-0011 v5 quarantine output lock",
        )
    if prepared.stage == "custody":
        return (
            CUSTODY_REPORT,
            {
                "schema_version": "treasurebench-fresh-pilot-v5-custody-commitment-v1",
            ...
            "Record AO-0011 v5 custody commitment",
        )
...
def _safe_existing_objects(root: Path, ledger: AppendOnlyLedger) -> dict[str, bytes]:
    ...
    for prefix, path_root, pattern in (
        ...
        (
            ...
            root / "encrypted-protocol-classifications",
            "*.sealed",
        ),
        ("metric-bound", root / "encrypted-metric-bounds", "*.sealed"),
...
def _quarantine_live_failure(
    ...
    *,
    ...
    summary: dict[str, object] = {
        "schema_version": "treasurebench-fresh-pilot-v5-quarantine-closeout-v1",
        "status": "quarantined",
        "decision": "fresh-pilot-v5-quarantined-engineering-only",
        "classification": "redacted-engineering-only-no-task-level-performance",
        "campaign_id": CAMPAIGN_ID,
        "batch_id": BATCH_ID,
        "stage": prepared.stage,
        ...
        try:
            ...
            result = _quarantine_live_failure(
                ...
                failure_class=(
                    ...
                    if isinstance(error, ProviderContractSafetyError)
                    else (
                        "operational-circuit-breaker"
                        if isinstance(error, OperationalCircuitBreakerError)
                        else _registered_failure_class(prepared.stage)
                    )
                ),
            )
            try:
...
def audit_live_corruptions(repo: Path) -> tuple[dict[str, str], ...]:
    """Reject v5 private-state, output-lock, and terminal-provider corruptions."""
    ...
    with tempfile.TemporaryDirectory(prefix="fresh-live-v5-corruptions-") as temporary:
        root = Path(temporary)
        _secure_directory(root)
        reject(
