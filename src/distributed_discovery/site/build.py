...
def _benchmark_pages(root: Path, output: Path) -> dict[str, object]:
    ...
    agents_repair = _read_yaml(
        root / "reports/benchmark/treasurebench-agents-v1-pilot-repair-adjudication.yml"
    )
    agents_repair_rehearsal = _read_yaml(
        root / "reports/benchmark/treasurebench-agents-v1-pilot-repair-rehearsal.yml"
    )
    agents_fresh_options = _read_yaml(
        root / "reports/benchmark/treasurebench-agents-v1-fresh-pilot-options.yml"
    )
    agents_fresh_closeout = _read_yaml(
        root / "reports/benchmark/treasurebench-agents-v1-fresh-pilot-closeout.yml"
    )
    agents_fresh_v2_closeout = _read_yaml(
        root / "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v2-closeout.yml"
    )
    agents_fresh_v3_closeout = _read_yaml(
        root / "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v3-closeout.yml"
    )
    agents_custody_closeout = _read_yaml(
        root / "reports/benchmark/treasurebench-ao0006-custody-diagnostic-closeout.yml"
    )
    agents_fixed_batch_outcome = _read_yaml(
        root / "reports/benchmark/treasurebench-ao0008-fixed-batch-diagnostic-r2-outcome.yml"
    )
    agents_policy_v2 = _read_yaml(root / "docs/benchmark/agents-v1/protocol-validity-policy-v2.yml")
    agents_fresh_v4_registration = _read_yaml(
        root / "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v4-registration.yml"
    )
    agents_fresh_v4_rehearsal = _read_yaml(
        root / "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v4-offline-rehearsal.yml"
    )
    agents_fresh_v4_scenarios = _read_yaml(
        root
        / "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v4-protocol-validity-scenarios.yml"
    )
    agents_fresh_v4_closeout = _read_yaml(
        root / "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v4-closeout.yml"
    )
    agents_policy_v3 = _read_yaml(root / "docs/benchmark/agents-v1/provider-outcome-policy-v3.yml")
    agents_fresh_v5_closeout = _read_yaml(
        root / "reports/benchmark/treasurebench-agents-v1-fresh-pilot-v5-closeout.yml"
    )
    ...
    if (
        ...
        or agents_repair.get("status") != "complete-redacted-engineering-adjudication"
        or agents_repair_rehearsal.get("status") != "pass"
        or agents_repair_rehearsal.get("corruptions_rejected") != 28
        or agents_fresh_options.get("status") != "options-complete-execution-not-authorized"
        or agents_fresh_closeout.get("decision") != "sealed-pilot-quarantined-provider-failure"
        or agents_fresh_closeout.get("executed", {}).get("attempts") != 1
        ...
        or agents_fresh_v2_closeout.get("decision") != "fresh-pilot-v2-quarantined-engineering-only"
        or agents_fresh_v2_closeout.get("executed", {}).get("stage") != "custody"
        or agents_fresh_v2_closeout.get("executed", {}).get("attempts") != 2
        or agents_fresh_v2_closeout.get("executed", {}).get("private_runs") != 0
        or agents_fresh_v2_closeout.get("executed", {}).get("cost_usd") != "0.0076095"
        or agents_fresh_v2_closeout.get("lock", {}).get("provider_phase_closed") is not True
        or agents_fresh_v2_closeout.get("lock", {}).get("output_lock_verified") is not True
        or agents_fresh_v3_closeout.get("decision") != "fresh-pilot-v3-quarantined-engineering-only"
        or agents_fresh_v3_closeout.get("executed", {}).get("stage") != "fixed-full-batch"
        or agents_fresh_v3_closeout.get("executed", {}).get("calls") != 3067
        or agents_fresh_v3_closeout.get("executed", {}).get("cost_usd") != "13.1861145"
        or agents_fresh_v3_closeout.get("lock", {}).get("objects_locked") != 3576
        or agents_fresh_v3_closeout.get("lock", {}).get("provider_phase_closed") is not True
        or agents_fresh_v3_closeout.get("lock", {}).get("output_lock_verified") is not True
        or agents_fresh_v3_closeout.get("lock", {}).get("minimum_unseal_performed") is not False
        or agents_custody_closeout.get("decision")
        != "custody-root-cause-repaired-live-conformance-pass"
        or agents_custody_closeout.get("diagnostic", {}).get("private_read_authority_closed")
        is not True
        or agents_custody_closeout.get("diagnostic", {}).get("retained_state_mutated") is not False
        or agents_custody_closeout.get("diagnostic", {}).get("provider_calls") != 0
        or agents_custody_closeout.get("diagnostic", {}).get("credential_reads") != 0
        or agents_custody_closeout.get("diagnostic", {}).get("spend_usd") != "0"
        or agents_custody_closeout.get("diagnosis", {}).get("exact_cause")
        != "v2-campaign-absent-from-private-generation-permit-allowlist"
        or agents_custody_closeout.get("conformance", {}).get("status") != "pass"
        or agents_custody_closeout.get("conformance", {}).get("required_classes") != 20
        or agents_custody_closeout.get("conformance", {}).get("required_classes_passed") != 20
        or agents_custody_closeout.get("conformance", {}).get("independent_verifier") != "pass"
        or agents_fixed_batch_outcome.get("decision") != "agent-protocol-policy-decision-required"
        or agents_fixed_batch_outcome.get("diagnostic", {}).get("authenticated_fixed_batch_traces")
        != 450
        or agents_fixed_batch_outcome.get("diagnostic", {}).get("private_read_authority_closed")
        is not True
        or agents_fixed_batch_outcome.get("diagnostic", {}).get("retained_state_mutated")
        is not False
        or agents_fixed_batch_outcome.get("aggregate_counters", {}).get(
            "protocol_nonconformance_traces"
        )
        != 32
        or agents_fixed_batch_outcome.get("aggregate_counters", {}).get(
        ...
        != 32
        or agents_fixed_batch_outcome.get("diagnosis", {}).get("causal_class")
        != "protocol-contract-nonconformance"
        or agents_fixed_batch_outcome.get("policy_disposition", {}).get(
            "agent_protocol_policy_decision_required"
        )
        is not True
        or agents_policy_v2.get("policy_version")
        != "treasurebench-agents-v1-protocol-validity-policy-v2"
        or agents_policy_v2.get("status") != "prospective-frozen-not-executed"
        or agents_policy_v2.get("authority_task") != "AO-0010"
        or agents_policy_v2.get("status_model", {}).get("statuses")
        != [
        ...
        or agents_policy_v2.get("batch_acceptance", {}).get(
            "protocol_invalid_alone_quarantines_batch"
        )
        is not False
        or agents_policy_v2.get("reporting", {}).get("provider_or_model_ranking") is not False
        or agents_fresh_v4_registration.get("status")
        != "pass-offline-exact-execution-frozen-owner-gate-required"
        or agents_fresh_v4_registration.get("task_id") != "AO-0010"
        or agents_fresh_v4_registration.get("counts", {}).get("runs") != 500
        or agents_fresh_v4_registration.get("activity", {}).get("provider_calls") != 0
        or agents_fresh_v4_registration.get("activity", {}).get("credential_reads") != 0
        or agents_fresh_v4_registration.get("activity", {}).get("spend_usd") != "0"
        or agents_fresh_v4_rehearsal.get("status") != "pass"
        or agents_fresh_v4_rehearsal.get("terminal_pairings_classified") != 500
        or agents_fresh_v4_rehearsal.get("metric_intervals") != 6400
        or agents_fresh_v4_rehearsal.get("architecture_contrast_bounds") != 120
        or agents_fresh_v4_rehearsal.get("independent_metric_bound_agreement") is not True
        or agents_fresh_v4_scenarios.get("status") != "pass"
        or agents_fresh_v4_scenarios.get("protocol_invalid_alone_quarantines") is not False
        or agents_fresh_v4_scenarios.get("provider_terminal_missing_quarantines") is not True
        or agents_fresh_v4_scenarios.get("pairing_integrity_failure_quarantines") is not True
        or agents_fresh_v4_scenarios.get("complete_case_overclaim_rejected") is not True
        or agents_fresh_v4_scenarios.get("scenarios", {})
        .get("all-invalid", {})
        .get("protocol_invalid")
        != 500
        or agents_fresh_v4_closeout.get("decision") != "fresh-pilot-v4-quarantined-engineering-only"
        or agents_fresh_v4_closeout.get("executed", {}).get("stage") != "private-prefix"
        or agents_fresh_v4_closeout.get("executed", {}).get("failure_class")
        != "provider-terminal-missing"
        or agents_fresh_v4_closeout.get("executed", {}).get("calls") != 116
        or agents_fresh_v4_closeout.get("executed", {}).get("cost_usd") != "0.3470610"
        or agents_fresh_v4_closeout.get("lock", {}).get("objects_locked") != 150
        or agents_fresh_v4_closeout.get("lock", {}).get("provider_phase_closed") is not True
        or agents_fresh_v4_closeout.get("lock", {}).get("output_lock_verified") is not True
        or agents_fresh_v4_closeout.get("lock", {}).get("minimum_unseal_performed") is not False
        or agents_fresh_v4_closeout.get("disposition", {}).get(
            "campaign_and_batch_permanently_quarantined"
        )
        is not True
        or agents_policy_v3.get("policy_version")
        != "treasurebench-agents-v1-provider-outcome-policy-v3"
        or agents_fresh_v5_closeout.get("decision")
        != "fresh-pilot-v5-engineering-complete-no-scientific-evidence"
        or agents_fresh_v5_closeout.get("executed", {}).get("intended_pairings_complete")
        is not True
        or agents_fresh_v5_closeout.get("executed", {}).get("terminal_pairings_classified") != 500
        or agents_fresh_v5_closeout.get("executed", {}).get("protocol_valid_pairings") != 496
        or agents_fresh_v5_closeout.get("executed", {}).get("protocol_invalid_pairings") != 4
        or agents_fresh_v5_closeout.get("executed", {}).get("provider_operational_missing_pairings")
        != 0
        or agents_fresh_v5_closeout.get("executed", {}).get(
            "provider_contract_or_safety_failure_pairings"
        )
        != 0
        or agents_fresh_v5_closeout.get("executed", {}).get("calls") != 3058
        or agents_fresh_v5_closeout.get("executed", {}).get("cost_usd") != "13.0413660"
        or agents_fresh_v5_closeout.get("retry_delay", {}).get("retry_delay_seconds") != 5
        or agents_fresh_v5_closeout.get("retry_delay", {}).get("completed_records") != 1
        or agents_fresh_v5_closeout.get("retry_delay", {}).get("third_transport_attempts") != 0
        or agents_fresh_v5_closeout.get("circuit_breaker", {}).get("fired") is not False
        or agents_fresh_v5_closeout.get("lock", {}).get("objects_locked") != 4067
        or agents_fresh_v5_closeout.get("lock", {}).get("provider_phase_closed") is not True
        or agents_fresh_v5_closeout.get("lock", {}).get("output_lock_verified") is not True
        or agents_fresh_v5_closeout.get("lock", {}).get("unseal_after_lock_verified") is not True
        or agents_fresh_v5_closeout.get("verification", {}).get(
            "independent_metric_bound_agreement"
        )
        is not True
        or agents_fresh_v5_closeout.get("redaction", {}).get("status") != "pass"
    ...
    agents_families = [
        {
            ...
            "name": str(item["family_id"]).replace("-", " ").title(),
        }
        for item in agents_registry["families"]
    ]
    agents_family_items = "".join(
        f"<li>{html.escape(str(item['name']))}</li>" for item in agents_families
    )
    agents_body = f"""
<header class="page-hero"><p class="eyebrow">DD-010 instrument · five quarantined pilots · v5 engineering complete · policies v2/v3</p><h1>DiscoveryBench Agents v1</h1><p class="lede">A complete instrument for studying how software-agent teams turn synthetic evidence into action portfolios. Its original sealed pilot and repair-confirmation v1 through v4 remain permanently quarantined. AO-0011 freezes provider-outcome policy v3 and completes one wholly fresh 500-pairing v5 engineering batch under unchanged protocol-validity policy v2. This is not a scientific evaluation or provider ranking.</p><p class="status-row"><span class="status-chip">Original pilot quarantined</span><span class="status-chip">Repair-confirmation v1 quarantined</span><span class="status-chip">Repair-confirmation v2 quarantined</span><span class="status-chip">Repair-confirmation v3 quarantined</span><span class="status-chip">Repair-confirmation v4 quarantined</span><span class="status-chip">Repair-confirmation v5 engineering complete</span><span class="status-chip">Provider schemas passed</span><span class="status-chip">Custody path repaired</span><span class="status-chip">Policies v2/v3 frozen</span></p></header>
...
<section class="content-section prose"><h2>Fresh repair-confirmation v2 closeout</h2><p>AO-0006 campaign <code>{html.escape(str(agents_fresh_v2_closeout["campaign_id"]))}</code>, batch <code>{html.escape(str(agents_fresh_v2_closeout["batch_id"]))}</code>, passed both direct-provider public canaries and then stopped at custody creation with decision <code>fresh-pilot-v2-quarantined-engineering-only</code> and public-safe class <code>custody-creation-failure</code>.</p><p>The two calls used 1,349 input tokens and 253 output tokens and cost USD 0.0076095: OpenAI USD 0.0027975 and Anthropic USD 0.004812. No private architecture/model run occurred. The provider phase is closed, and eight safely lockable objects are preserved under output lock <code>{html.escape(str(agents_fresh_v2_closeout["lock"]["output_lock_commitment"]))}</code>. No call followed the lock and no material was unsealed.</p><p>A new seed and task and answer keys exist only in retained private state; no task ciphertext, answer ciphertext, or custody manifest was created. The campaign and batch cannot be retried, repaired, reopened, reused, rescored, spliced, executed, or reauthorized.</p></section>
<section class="content-section prose"><h2>AO-0007 custody diagnosis and prospective repair</h2><p>The one-use, owner-authorized AO-0007 diagnostic verified the AO-0006 retained boundary without mutation, identified <code>{html.escape(str(agents_custody_closeout["diagnosis"]["exact_cause"]))}</code> as the exact custody failure, and permanently closed private-read authority. It disclosed no private value or host path and made zero credential reads, provider calls, or spend.</p><p>The repair is prospective only: AO-0006 remains quarantined and unchanged. The production custody path now passes all {int(agents_custody_closeout["conformance"]["required_classes_passed"])} required synthetic live-mode classes, independent {html.escape(str(agents_custody_closeout["conformance"]["algorithm"]))} verification, and six negative classes. No new private pilot, scientific run, claim, performance evidence, ranking, or base-campaign authority was created.</p></section>
<section class="content-section prose"><h2>Fresh repair-confirmation v3 closeout</h2><p>AO-0008 campaign <code>{html.escape(str(agents_fresh_v3_closeout["campaign_id"]))}</code>, batch <code>{html.escape(str(agents_fresh_v3_closeout["batch_id"]))}</code>, passed both direct-provider public canaries, wholly fresh custody, and the private prefix. The fixed full batch then stopped with decision <code>fresh-pilot-v3-quarantined-engineering-only</code> and public-safe class <code>fixed-full-batch-failure</code>.</p><p>The provider phase made 3,067 calls, used 2,304,303 input tokens and 444,085 output tokens, and cost USD 13.1861145: OpenAI USD 4.5952575 and Anthropic USD 8.590857. Every hard cap held. Provider access is closed, and 3,576 retained objects are preserved under output lock <code>{html.escape(str(agents_fresh_v3_closeout["lock"]["output_lock_commitment"]))}</code>. No call followed the lock and no material was unsealed.</p><p>The partial private batch is not evaluated or published. The campaign and batch cannot be retried, repaired, reopened, reused, rescored, spliced, executed, or reauthorized.</p></section>
<section class="content-section prose"><h2>AO-0009 aggregate fixed-batch diagnosis</h2><p>The exact R2 owner-gated diagnostic ran once, authenticated all {int(agents_fixed_batch_outcome["diagnostic"]["authenticated_fixed_batch_traces"])} fixed-full-batch traces, verified the output lock, immutable inventory, append-only ledgers, complete response identity correspondence, unique 2/50/450 trace partition, all 500 pairing records, and retained-state immutability, then permanently closed private-read authority.</p><p>The public aggregate reports {int(agents_fixed_batch_outcome["aggregate_counters"]["protocol_nonconformance_traces"])} protocol-nonconforming traces and {int(agents_fixed_batch_outcome["aggregate_counters"]["parse_or_schema_repair_exhaustion_traces"])} parse/schema-repair exhaustion traces. Terminal-provider attempts, contamination traces, invalid final cardinalities, cap triggers, and integrity failures are zero. Frozen causal precedence selects <code>{html.escape(str(agents_fixed_batch_outcome["decision"]))}</code>.</p><p>AO-0009 made no protocol acceptance, normalization, retry, invalid-output credit, or scoring change. Any prospective agent-protocol change required a separate explicit owner decision and task. AO-0008 remains permanently quarantined, private-read authority is closed, and at AO-0009 closeout v4 was not registered.</p></section>
<section class="content-section prose"><h2>AO-0010 policy v2 and fresh v4 closeout</h2><p>AO-0010 registers forward-only policy <code>{html.escape(str(agents_policy_v2["policy_version"]))}</code> and wholly fresh campaign <code>{html.escape(str(agents_fresh_v4_registration["campaign_id"]))}</code>, batch <code>{html.escape(str(agents_fresh_v4_registration["batch_id"]))}</code>. Historical policies, schemas, identities, locks, and decisions remain unchanged.</p><p>The policy separates batch-integrity validity, missing provider terminals, protocol-valid output, and protocol-invalid output. A completed response still nonconforming after one schema-only repair remains in every intended-pairing denominator without replacement, semantic retry, invented action, or invalid-output credit. Protocol-invalid traces alone do not quarantine. Registered metric-specific feasible bounds and architecture-contrast bounds cover all intended pairings; complete-case effects cannot be presented as unconditional.</p><p>The authorization-free 50-task/500-pairing rehearsal classifies all {int(agents_fresh_v4_rehearsal["terminal_pairings_classified"])} pairings, constructs {int(agents_fresh_v4_rehearsal["metric_intervals"]):,} pairing metric intervals and {int(agents_fresh_v4_rehearsal["architecture_contrast_bounds"])} architecture-contrast bounds, and passes independent reconstruction. Zero, mixed, and all-invalid synthetic batches pass without protocol-invalid-only quarantine; provider-terminal and integrity failures still quarantine. All 104 relevant corruptions reject.</p><p>The exact authorized v4 attempt passed both public canaries and wholly fresh custody, then stopped during the private prefix under class <code>{html.escape(str(agents_fresh_v4_closeout["executed"]["failure_class"]))}</code>. Provider access closed after {int(agents_fresh_v4_closeout["executed"]["calls"]):,} calls, {int(agents_fresh_v4_closeout["executed"]["input_tokens"]):,} input tokens, {int(agents_fresh_v4_closeout["executed"]["output_tokens"]):,} output tokens, and USD {html.escape(str(agents_fresh_v4_closeout["executed"]["cost_usd"]))}. The verified lock preserves {int(agents_fresh_v4_closeout["lock"]["objects_locked"]):,} retained objects under <code>{html.escape(str(agents_fresh_v4_closeout["lock"]["output_lock_commitment"]))}</code>. No call followed the lock and no material was unsealed.</p><p>The incomplete real batch has no protocol classification, Methods A/B/C, metric bounds, or performance interpretation. V4 is permanently quarantined and cannot be retried, replaced, repaired, reopened, reused, rescored, spliced, executed, unsealed, or reauthorized. No performance result, scientific run, claim, ranking, release, submission, or base campaign is created.</p></section>
<section class="content-section prose"><h2>AO-0011 policy v3 and fresh v5 closeout</h2><p>AO-0011 freezes forward-only policy <code>{html.escape(str(agents_policy_v3["policy_version"]))}</code> under unchanged protocol-validity policy v2 and completes campaign <code>{html.escape(str(agents_fresh_v5_closeout["campaign_id"]))}</code>, batch <code>{html.escape(str(agents_fresh_v5_closeout["batch_id"]))}</code>.</p><p>Both public canaries, wholly fresh 50-task custody, the private prefix, and all {int(agents_fresh_v5_closeout["executed"]["terminal_pairings_classified"])} intended pairings completed. Terminal classification contains {int(agents_fresh_v5_closeout["executed"]["protocol_valid_pairings"])} protocol-valid and {int(agents_fresh_v5_closeout["executed"]["protocol_invalid_pairings"])} protocol-invalid pairings, zero provider-operational missingness, zero contract/safety failure, and no circuit-breaker trigger.</p><p>One transient-provider first attempt selected and completed the frozen {int(agents_fresh_v5_closeout["retry_delay"]["retry_delay_seconds"])}-second fallback before the sole identical retry recovered. No third, changed, duplicate, semantic, outcome-dependent, or replacement retry occurred. Provider access closed after {int(agents_fresh_v5_closeout["executed"]["calls"]):,} calls, {int(agents_fresh_v5_closeout["executed"]["input_tokens"]):,} input tokens, {int(agents_fresh_v5_closeout["executed"]["output_tokens"]):,} output tokens, and USD {html.escape(str(agents_fresh_v5_closeout["executed"]["cost_usd"]))}.</p><p>The verified output lock covers {int(agents_fresh_v5_closeout["lock"]["objects_locked"]):,} objects under <code>{html.escape(str(agents_fresh_v5_closeout["lock"]["output_lock_commitment"]))}</code>. No call followed the lock. Post-lock replay/unseal, Methods A/B/C, independent provider classification and all-pairing bounds, exact identity and cost correspondence, contamination, corruptions, redaction, and retained-state checks pass.</p><p>This is aggregate engineering status only. No task, answer, prompt, provider output, raw trace, task-level metric, model/provider/architecture/family performance, comparison, ranking, composite, scientific run, claim, paper result, release, submission, DD-023, or base campaign is published or created.</p></section>
...
<section class="content-section prose"><h2>Versions and next gate</h2><dl><div><dt>Owner</dt><dd>DD-010 instrument</dd></div><div><dt>Content</dt><dd>Explicit selection from preserved v1, v2, or v3</dd></div><div><dt>Agent protocol</dt><dd><code>{html.escape(str(agents_versions["axes"]["agent_protocol"]))}</code></dd></div><div><dt>Protocol-validity policy</dt><dd><code>{html.escape(str(agents_policy_v2["policy_version"]))}</code></dd></div><div><dt>Provider-outcome policy</dt><dd><code>{html.escape(str(agents_policy_v3["policy_version"]))}</code></dd></div><div><dt>Generator</dt><dd><code>{html.escape(str(agents_versions["axes"]["task_generator"]))}</code></dd></div><div><dt>Original pilot lock</dt><dd><code>{html.escape(str(agents_pilot["output_lock_commitment"]))}</code></dd></div><div><dt>Repair-confirmation v1 lock</dt><dd><code>{html.escape(str(agents_fresh_closeout["lock"]["output_lock_commitment"]))}</code></dd></div><div><dt>Repair-confirmation v2 lock</dt><dd><code>{html.escape(str(agents_fresh_v2_closeout["lock"]["output_lock_commitment"]))}</code></dd></div><div><dt>Repair-confirmation v3 lock</dt><dd><code>{html.escape(str(agents_fresh_v3_closeout["lock"]["output_lock_commitment"]))}</code></dd></div><div><dt>Repair-confirmation v4 lock</dt><dd><code>{html.escape(str(agents_fresh_v4_closeout["lock"]["output_lock_commitment"]))}</code></dd></div><div><dt>Repair-confirmation v5 lock</dt><dd><code>{html.escape(str(agents_fresh_v5_closeout["lock"]["output_lock_commitment"]))}</code></dd></div></dl><p>The custody path is repaired, policies v2 and v3 are frozen, the original pilot and v1 through v4 remain permanently quarantined, and v5 is engineering complete. Any separate future task requires a new explicit owner decision and wholly new identities. The 200-task base campaign remains blocked.</p><p><a href="../data/benchmark/agents-v1-provider-schema-conformance.json">Download the provider-schema conformance summary</a> · <a href="../data/benchmark/agents-v1-evaluation.json">Download the redacted operational summary</a> · <a href="../data/benchmark/agents-v1-implementation.json">Download the implementation summary</a> · <a href="../data/benchmark/agents-v1-registration.json">Download the preserved instrument registration</a> · <a href="../research/dd-010.html">DD-010 study record</a> · <a href="../benchmark.html">DiscoveryBench overview</a></p></section>
"""
    _write_benchmark_route_pair(
        output,
    ...
    agents_public_data = {
        ...
        "benchmark_content_versions": ["v1", "v2", "v3"],
        "agent_protocol_version": "agents-v1",
        "generator_version": "agents-task-generator-v1",
        "task_families": agents_families,
        "boundaries": {
            "model_calls": 0,
            "cost_incurred": False,
        ...
        "next_gate": "DiscoveryBench Agents v1 offline instrument implementation",
    }
    _write(
        output,
    ...
    agents_implementation_data = {
        ...
        "repair_rehearsal": {
            "status": agents_repair_rehearsal["status"],
            "cases": agents_repair_rehearsal["cases"],
            "method_a_b_agree": True,
            "method_c_errors": agents_repair_rehearsal["method_c_errors"],
            "corruptions_rejected": agents_repair_rehearsal["corruptions_rejected"],
            "rehearsal_hash": agents_repair_rehearsal["stable_rehearsal_hash"],
        },
        "custody_path_conformance": {
            "classification": agents_custody_closeout["classification"],
            "decision": agents_custody_closeout["decision"],
            "exact_cause": agents_custody_closeout["diagnosis"]["exact_cause"],
            "required_classes": agents_custody_closeout["conformance"]["required_classes"],
            "required_classes_passed": agents_custody_closeout["conformance"][
                "required_classes_passed"
            ],
            "independent_verifier": agents_custody_closeout["conformance"]["independent_verifier"],
            "algorithm": agents_custody_closeout["conformance"]["algorithm"],
            "negative_classes": len(agents_custody_closeout["conformance"]["corruptions"]),
            "provider_calls": agents_custody_closeout["diagnostic"]["provider_calls"],
            "cost_usd": agents_custody_closeout["diagnostic"]["spend_usd"],
            "private_read_authority_closed": agents_custody_closeout["diagnostic"][
                "private_read_authority_closed"
            ],
        },
        "fixed_batch_diagnostic": {
            "classification": agents_fixed_batch_outcome["classification"],
            "decision": agents_fixed_batch_outcome["decision"],
            "authenticated_fixed_batch_traces": agents_fixed_batch_outcome["diagnostic"][
                "authenticated_fixed_batch_traces"
            ],
            "protocol_nonconformance_traces": agents_fixed_batch_outcome["aggregate_counters"][
                "protocol_nonconformance_traces"
            ],
            "parse_or_schema_repair_exhaustion_traces": agents_fixed_batch_outcome[
                "aggregate_counters"
            ]["parse_or_schema_repair_exhaustion_traces"],
            "causal_class": agents_fixed_batch_outcome["diagnosis"]["causal_class"],
            "private_read_authority_closed": agents_fixed_batch_outcome["diagnostic"][
                "private_read_authority_closed"
            ],
            "retained_state_mutated": agents_fixed_batch_outcome["diagnostic"][
                "retained_state_mutated"
            ],
            "provider_calls": agents_fixed_batch_outcome["diagnostic"]["provider_calls"],
            "cost_usd": agents_fixed_batch_outcome["diagnostic"]["spend_usd"],
        },
        "prospective_policy_v2": {
            "policy_version": agents_policy_v2["policy_version"],
            "status": agents_policy_v2["status"],
            "statuses": agents_policy_v2["status_model"]["statuses"],
            "protocol_invalid_alone_quarantines": agents_policy_v2["batch_acceptance"][
                "protocol_invalid_alone_quarantines_batch"
            ],
            "global_protocol_conformance_threshold": agents_policy_v2["batch_acceptance"][
                "global_protocol_conformance_threshold"
            ],
            "provider_or_model_ranking": agents_policy_v2["reporting"]["provider_or_model_ranking"],
        },
        "fresh_v4_preparation": {
            "task_id": agents_fresh_v4_registration["task_id"],
            "campaign_id": agents_fresh_v4_registration["campaign_id"],
            "batch_id": agents_fresh_v4_registration["batch_id"],
            "status": agents_fresh_v4_registration["status"],
            "tasks": agents_fresh_v4_rehearsal["tasks"],
            "intended_pairings": agents_fresh_v4_rehearsal["runs"],
            "terminal_pairings_classified": agents_fresh_v4_rehearsal[
                "terminal_pairings_classified"
            ],
            "metric_intervals": agents_fresh_v4_rehearsal["metric_intervals"],
            "architecture_contrast_bounds": agents_fresh_v4_rehearsal[
                "architecture_contrast_bounds"
            ],
            "zero_mixed_all_invalid_scenarios": "pass",
            "total_relevant_corruptions_rejected": 104,
            "provider_calls": agents_fresh_v4_registration["activity"]["provider_calls"],
            "credential_reads": agents_fresh_v4_registration["activity"]["credential_reads"],
            "external_cost_usd": agents_fresh_v4_registration["activity"]["spend_usd"],
            "owner_gate_required": False,
        },
        "fresh_v4_closeout": {
            "decision": agents_fresh_v4_closeout["decision"],
            "campaign_id": agents_fresh_v4_closeout["campaign_id"],
            "batch_id": agents_fresh_v4_closeout["batch_id"],
            "stage": agents_fresh_v4_closeout["executed"]["stage"],
            "failure_class": agents_fresh_v4_closeout["executed"]["failure_class"],
            "calls": agents_fresh_v4_closeout["executed"]["calls"],
            "input_tokens": agents_fresh_v4_closeout["executed"]["input_tokens"],
            "output_tokens": agents_fresh_v4_closeout["executed"]["output_tokens"],
            "cost_usd": agents_fresh_v4_closeout["executed"]["cost_usd"],
            "objects_locked": agents_fresh_v4_closeout["lock"]["objects_locked"],
            "output_lock_commitment": agents_fresh_v4_closeout["lock"]["output_lock_commitment"],
            "provider_phase_closed": agents_fresh_v4_closeout["lock"]["provider_phase_closed"],
            "minimum_unseal_performed": agents_fresh_v4_closeout["lock"][
                "minimum_unseal_performed"
            ],
            "real_batch_protocol_classification": agents_fresh_v4_closeout["verification"][
                "real_batch_protocol_classification"
            ],
            "performance_results_exist": False,
        },
        "fresh_v5_closeout": {
            "decision": agents_fresh_v5_closeout["decision"],
            "campaign_id": agents_fresh_v5_closeout["campaign_id"],
            "batch_id": agents_fresh_v5_closeout["batch_id"],
            "terminal_pairings_classified": agents_fresh_v5_closeout["executed"][
                "terminal_pairings_classified"
            ],
            "protocol_valid_pairings": agents_fresh_v5_closeout["executed"][
                "protocol_valid_pairings"
            ],
            "protocol_invalid_pairings": agents_fresh_v5_closeout["executed"][
                "protocol_invalid_pairings"
            ],
            "provider_operational_missing_pairings": agents_fresh_v5_closeout["executed"][
                "provider_operational_missing_pairings"
            ],
            "provider_contract_or_safety_failure_pairings": agents_fresh_v5_closeout["executed"][
                "provider_contract_or_safety_failure_pairings"
            ],
            "calls": agents_fresh_v5_closeout["executed"]["calls"],
            "input_tokens": agents_fresh_v5_closeout["executed"]["input_tokens"],
            "output_tokens": agents_fresh_v5_closeout["executed"]["output_tokens"],
            "cost_usd": agents_fresh_v5_closeout["executed"]["cost_usd"],
            "retry_delay_seconds": agents_fresh_v5_closeout["retry_delay"]["retry_delay_seconds"],
            "third_transport_attempts": agents_fresh_v5_closeout["retry_delay"][
                "third_transport_attempts"
            ],
            "circuit_breaker_fired": agents_fresh_v5_closeout["circuit_breaker"]["fired"],
            "objects_locked": agents_fresh_v5_closeout["lock"]["objects_locked"],
            "output_lock_commitment": agents_fresh_v5_closeout["lock"]["output_lock_commitment"],
            "provider_phase_closed": agents_fresh_v5_closeout["lock"]["provider_phase_closed"],
            "unseal_after_lock_verified": agents_fresh_v5_closeout["lock"][
                "unseal_after_lock_verified"
            ],
            "independent_metric_bound_agreement": agents_fresh_v5_closeout["verification"][
                "independent_metric_bound_agreement"
            ],
            "performance_results_exist": False,
        },
        "boundaries": {
            "provider_calls": agents_fresh_v5_closeout["executed"]["calls"],
            "model_invocations": agents_fresh_v5_closeout["executed"]["calls"],
            "model_downloads": 0,
            "network_calls": agents_fresh_v5_closeout["executed"]["calls"],
            "external_cost_usd": agents_fresh_v5_closeout["executed"]["cost_usd"],
            "private_material_exists": True,
            "performance_results_exist": False,
            "leaderboard": False,
            "composite_score": None,
        },
        "future_authorization_required": False,
        "campaign_registration": "fresh-v5-engineering-complete-prior-campaigns-preserved",
        ...
        "next_gate": (
            "new explicit owner decision and wholly new identities required for "
            "any separate future task"
        ),
    }
    _write(
        output,
    ...
    agents_evaluation_data = {
        ...
        "public_calibration": {
            ...
            "cumulative_calls_across_attempts": int(agents_ledger["calls_made"]),
            "cumulative_cost_usd": agents_cost,
        ...
        "repair_adjudication": {
            "status": agents_repair["status"],
            "decision": agents_repair["decision"]["repair_decision"],
            "private_run_traces_inspected": agents_repair["verification"][
                "private_run_traces_inspected"
            ],
            "runs_with_invalid_final_cardinality": agents_repair["action_budget"][
                "runs_with_invalid_final_cardinality"
            ],
            "invalid_final_agent_outputs": agents_repair["action_budget"][
                "invalid_final_agent_outputs"
            ],
            "over_budget_final_agent_outputs": agents_repair["action_budget"][
                "over_budget_final_agent_outputs"
            ],
            "metric_records_changed_by_extra_action_credit": agents_repair["action_budget"][
                "metric_records_changed_by_extra_action_credit"
            ],
            "legacy_coverage_range_violations": agents_repair["action_budget"][
                "legacy_coverage_range_violations"
            ],
            "provider_events": [
                {
                    "provider": item["provider"],
                    "coarse_error_class": item["coarse_error_class"],
                    "taxonomy_class": item["taxonomy_class"],
                    "status": item["status"],
                    "terminal_run_missing": item["terminal_run_missing"],
                }
                for item in agents_repair["provider_error_adjudication"]["events"]
            ],
            "protocol_invalid_runs_attributable_to_terminal_provider_missingness": (
                agents_repair["protocol_relationship"][
                    "protocol_invalid_runs_attributable_to_terminal_provider_missingness"
                ]
            ),
        ...
        "repair_confirmation_pilot": {
            "name": "fresh-repair-confirmation-pilot",
            "campaign_id": agents_fresh_closeout["campaign_id"],
            "batch_id": agents_fresh_closeout["batch_id"],
            "execution_commit": agents_fresh_closeout["execution_commit"],
            "status": "quarantined",
            "decision": agents_fresh_closeout["decision"],
            "stage": agents_fresh_closeout["executed"]["stage"],
            "attempts": agents_fresh_closeout["executed"]["attempts"],
            "successes": agents_fresh_closeout["executed"]["successes"],
            "errors": agents_fresh_closeout["executed"]["errors"],
            "error_class": "schema-or-parameter",
            "terminal_http_status": agents_fresh_closeout["executed"]["terminal_http_status"],
            "openai_calls": agents_fresh_closeout["executed"]["OpenAI_calls"],
            "anthropic_calls": agents_fresh_closeout["executed"]["Anthropic_calls"],
            "input_tokens": agents_fresh_closeout["executed"]["input_tokens"],
            "output_tokens": agents_fresh_closeout["executed"]["output_tokens"],
            "cost_usd": agents_fresh_closeout["executed"]["cost_usd"],
            "private_runs": agents_fresh_closeout["executed"]["private_runs"],
            "task_seed_created": agents_fresh_closeout["custody"]["os_csprng_task_seed_created"],
            "tasks_created": agents_fresh_closeout["custody"]["tasks_created"],
            "answers_created": agents_fresh_closeout["custody"]["answers_created"],
            "task_key_created": agents_fresh_closeout["custody"]["task_key_created"],
            "answer_key_created": agents_fresh_closeout["custody"]["answer_key_created"],
            "task_ciphertext_created": agents_fresh_closeout["custody"]["task_ciphertext_created"],
            "answer_ciphertext_created": agents_fresh_closeout["custody"][
                "answer_ciphertext_created"
            ],
            "custody_manifest_created": agents_fresh_closeout["custody"][
                "custody_manifest_created"
            ],
            "output_lock_commitment": agents_fresh_closeout["lock"]["output_lock_commitment"],
            "provider_phase_closed": agents_fresh_closeout["lock"]["provider_phase_closed"],
            "authorization_inactive_archived": True,
            "performance_results_published": False,
        },
        "repair_confirmation_pilot_v2": {
            "name": "fresh-repair-confirmation-pilot-v2",
            "campaign_id": agents_fresh_v2_closeout["campaign_id"],
            "batch_id": agents_fresh_v2_closeout["batch_id"],
            "execution_commit": agents_fresh_v2_closeout["execution_commit"],
            "status": "quarantined",
            "decision": agents_fresh_v2_closeout["decision"],
            "stage": agents_fresh_v2_closeout["executed"]["stage"],
            "failure_class": agents_fresh_v2_closeout["executed"]["failure_class"],
            "attempts": agents_fresh_v2_closeout["executed"]["attempts"],
            "successes": agents_fresh_v2_closeout["executed"]["successes"],
            "errors": agents_fresh_v2_closeout["executed"]["errors"],
            "openai_calls": agents_fresh_v2_closeout["executed"]["OpenAI_calls"],
            "anthropic_calls": agents_fresh_v2_closeout["executed"]["Anthropic_calls"],
            "input_tokens": agents_fresh_v2_closeout["executed"]["input_tokens"],
            "output_tokens": agents_fresh_v2_closeout["executed"]["output_tokens"],
            "cost_usd": agents_fresh_v2_closeout["executed"]["cost_usd"],
            "provider_cost_usd": agents_fresh_v2_closeout["executed"]["provider_cost_usd"],
            "private_runs": agents_fresh_v2_closeout["executed"]["private_runs"],
            "task_seed_created": agents_fresh_v2_closeout["custody"]["os_csprng_seed_created"],
            "task_key_created": agents_fresh_v2_closeout["custody"]["task_key_created"],
            "answer_key_created": agents_fresh_v2_closeout["custody"]["answer_key_created"],
            "task_ciphertext_created": agents_fresh_v2_closeout["custody"][
                "task_ciphertext_created"
            ],
            "answer_ciphertext_created": agents_fresh_v2_closeout["custody"][
                "answer_ciphertext_created"
            ],
            "custody_manifest_created": agents_fresh_v2_closeout["custody"][
                "custody_manifest_created"
            ],
            "output_lock_commitment": agents_fresh_v2_closeout["lock"]["output_lock_commitment"],
            "objects_locked": agents_fresh_v2_closeout["lock"]["objects_locked"],
            "provider_phase_closed": agents_fresh_v2_closeout["lock"]["provider_phase_closed"],
            "unsealed": agents_fresh_v2_closeout["lock"]["minimum_unseal_performed"],
            "performance_results_published": False,
        },
        "repair_confirmation_pilot_v3": {
            "name": "fresh-repair-confirmation-pilot-v3",
            "campaign_id": agents_fresh_v3_closeout["campaign_id"],
            "batch_id": agents_fresh_v3_closeout["batch_id"],
            "execution_commit": agents_fresh_v3_closeout["execution_commit"],
            "status": "quarantined",
            "decision": agents_fresh_v3_closeout["decision"],
            "stage": agents_fresh_v3_closeout["executed"]["stage"],
            "failure_class": agents_fresh_v3_closeout["executed"]["failure_class"],
            "calls": agents_fresh_v3_closeout["executed"]["calls"],
            "input_tokens": agents_fresh_v3_closeout["executed"]["input_tokens"],
            "output_tokens": agents_fresh_v3_closeout["executed"]["output_tokens"],
            "cost_usd": agents_fresh_v3_closeout["executed"]["cost_usd"],
            "provider_cost_usd": agents_fresh_v3_closeout["executed"]["provider_cost_usd"],
            "public_canary_complete": agents_fresh_v3_closeout["executed"][
                "public_canary_complete"
            ],
            "custody_complete": agents_fresh_v3_closeout["executed"]["custody_complete"],
            "private_prefix_complete": agents_fresh_v3_closeout["executed"][
                "private_prefix_complete"
            ],
            "fixed_full_batch_complete": agents_fresh_v3_closeout["executed"][
                "fixed_full_batch_complete"
            ],
            "partial_private_pairing_count": agents_fresh_v3_closeout["executed"][
                "partial_private_pairing_count"
            ],
            "output_lock_commitment": agents_fresh_v3_closeout["lock"]["output_lock_commitment"],
            "objects_locked": agents_fresh_v3_closeout["lock"]["objects_locked"],
            "provider_phase_closed": agents_fresh_v3_closeout["lock"]["provider_phase_closed"],
            "unsealed": agents_fresh_v3_closeout["lock"]["minimum_unseal_performed"],
            "performance_results_published": False,
        },
        "provider_schema_conformance": {
        ...
        "future_base": {
            ...
            "boundary_repeats": 62,
            "repeats": 3,
            "status": "blocked-local-open-host-ineligible",
        },
        "models": {
            "cloud_snapshots": ["gpt-5.4-2026-03-05", "claude-sonnet-4-6"],
            "local_revision": (
                "mistralai/Mistral-Small-3.1-24B-Instruct-2503"
                "@68faf511d618ef198fef186659617cfd2eb8e33a"
            ),
            "public_calibration_calls_cumulative": int(agents_ledger["calls_made"]),
            "downloads": 0,
        },
        "authorization": {
            ...
            "original_pilot_cap_usd": 100,
            "fresh_pilot_authorized": False,
            "fresh_pilot_completed": True,
            "fresh_pilot_decision": agents_fresh_closeout["decision"],
            "fresh_pilot_expected_cost_usd": "15.00",
            "fresh_pilot_proposed_cap_usd": "25.00",
            "fresh_pilot_proposed_provider_caps_usd": {
                "OpenAI": "10.00",
                "Anthropic": "15.00",
            },
            "fresh_pilot_v2_authorized": True,
            "fresh_pilot_v2_completed": True,
            "fresh_pilot_v2_decision": agents_fresh_v2_closeout["decision"],
            "fresh_pilot_v2_expected_cost_usd": "11.51",
            "fresh_pilot_v2_hard_cap_usd": "25",
            "fresh_pilot_v3_authorized": True,
            "fresh_pilot_v3_completed": True,
            "fresh_pilot_v3_decision": agents_fresh_v3_closeout["decision"],
            "fresh_pilot_v3_cost_usd": agents_fresh_v3_closeout["executed"]["cost_usd"],
            "fresh_pilot_v3_hard_cap_usd": "25",
        ...
        "next_gate": (
            "no replacement private pilot selected; explicit owner decision required; "
            "base campaign blocked"
        ),
    }
    _write(
        output,
    ...
    agents_provider_conformance_data = {
        ...
        "next_candidate_registered": True,
        "next_candidate_authorized": True,
        "next_candidate_decision": agents_fresh_v2_closeout["decision"],
        "further_candidate_selected": False,
    }
    _write(
        output,
...
<section class="content-section prose"><h2>How the work is published</h2><p>The canonical entry paper introduces the atomic paradox. Theorem-family papers own durable mathematical questions. Working notes support intermediate or synthetic arguments. The living synthesis preserves the complete intellectual account. Reproducible studies, Labs, and DiscoveryBench expose evidence and interfaces.</p><p>No journal submission status is represented here. A validated PDF, exact run, or polished site does not by itself satisfy the paper-admission rule.</p><p><a href="publications.html">Browse working papers</a> · <a href="{REPOSITORY_URL}/blob/main/docs/research-governance.md">Read the governance source</a></p></section>"""
    program_body = program_body.replace(
        '<section class="content-section prose"><h2>Next open boundary</h2><p>The bounded Decentralized Recovery registration gate stopped at classical overlap: the frozen equal-sharing action game is singleton congestion, and visible sequential occupancy adds ordinary backward induction without enlarging the every-equilibrium top-two recovery region. No study, claim, run, or paper was created.</p><p>Reliable Discovery is now the next unregistered theorem program. It asks when reliable, unreliable, repeated, or overlapping actions should diversify or concentrate, and must establish content beyond classical reliability allocation before registration. The Price of Missing Provenance follows separately.</p></section>',
        '<section class="content-section prose"><h2>Phase boundary and hold</h2><p>Phase 1 is complete: Programs V1–V5, this Frontier, post-V5 consolidation, and the stopped decentralized-recovery overlap gate form the completed boundary. This does not mean every theorem direction is complete.</p><p>Phase 2 holds theorem-family execution. Reliable Discovery remains a major candidate but is deferred. <a href="benchmark/agents-v1.html">DiscoveryBench Agents v1</a> has five permanently quarantined DD-010 engineering pilots plus one engineering-complete v5 batch. The original pilot repair adjudication found a pervasive final-action-budget defect and repaired the instrument. Repair-confirmation v1 then stopped with decision <code>sealed-pilot-quarantined-provider-failure</code> on its first public canary: one OpenAI HTTP 400, zero Anthropic calls, zero reported tokens, USD 0.00, and no private run. The separate AO-0004 R4 public-only provider-schema conformance gate passed both providers’ minimal and complete action schemas plus the canonical exactly-one-final-action semantic contract in four calls costing USD 0.0086685, with no bisection and no private or scientific state change. The wholly fresh v2 campaign subsequently passed both public canaries in two calls costing USD 0.0076095, then quarantined at custody creation with zero private runs, a closed provider phase, and a verified eight-object output lock. AO-0008 then passed both public canaries, wholly fresh custody, and its private prefix before quarantining during the fixed full batch at 3,067 calls and USD 13.1861145, with a closed provider phase, a verified 3,576-object output lock, and no unseal. AO-0009’s exactly once aggregate diagnostic subsequently authenticated all 450 fixed-batch traces, permanently closed private-read authority, and selected <code>agent-protocol-policy-decision-required</code> without changing the fail-closed protocol. AO-0010 froze protocol-validity policy v2, passed its complete 500-pairing public rehearsal and all-pairing bounds, then ran one wholly fresh v4 engineering attempt. V4 passed both public canaries and wholly fresh custody before quarantining during the private prefix on <code>provider-terminal-missing</code> at 116 calls and USD 0.3470610, with a closed provider phase, a verified 150-object output lock, and no unseal. AO-0011 then froze provider-outcome policy v3 and completed one wholly fresh v5 engineering batch: 500 terminal pairings comprise 496 protocol-valid and four protocol-invalid outcomes, with zero provider-operational missingness, zero contract/safety failure, no circuit breaker, one safely recovered five-second retry, 3,058 calls, USD 13.0413660, a closed provider phase, a verified 4,067-object lock, and post-lock verification. These are engineering dispositions, not scientific or model-performance evidence. No study, claim, scientific run, paper result, ranking, dimension breakdown, or detailed private performance is published. The base campaign remains blocked. <a href="start-here.html">Start with three results</a> · <a href="methods.html">Read the factual methods record</a>.</p></section>',
        1,
    )
    program_body = program_body.replace(
        "The wholly fresh v2 campaign subsequently passed both public canaries in two calls "
        "costing USD 0.0076095, then quarantined at custody creation with zero private runs, "
        "a closed provider phase, and a verified eight-object output lock.",
        "The wholly fresh v2 campaign subsequently passed both public canaries in two calls "
        "costing USD 0.0076095, then quarantined at custody creation with zero private runs, "
        "a closed provider phase, and a verified eight-object output lock. AO-0007’s one-use "
        "diagnostic subsequently verified the retained boundary without mutation, identified "
        "the missing v2 campaign permit as the exact custody failure, and permanently closed "
        "private-read authority. The prospectively repaired production path passes all twenty "
        "synthetic live-mode custody classes and independent AES-256-GCM verification.",
        1,
    )
