...
def _benchmark_pages(root: Path, output: Path) -> dict[str, object]:
    ...
    agents_body = f"""
<header class="page-hero"><p class="eyebrow">DD-010 instrument · quarantined pilots · custody path repaired</p><h1>DiscoveryBench Agents v1</h1><p class="lede">A complete instrument for studying how software-agent teams turn synthetic evidence into action portfolios. Its original sealed pilot and both fresh repair-confirmation campaigns are permanently quarantined, while public provider-schema, semantic-contract, and prospective custody-path conformance passed. This is not a scientific evaluation or provider ranking.</p><p class="status-row"><span class="status-chip">Original pilot quarantined</span><span class="status-chip">Repair-confirmation v1 quarantined</span><span class="status-chip">Repair-confirmation v2 quarantined</span><span class="status-chip">Provider schemas passed</span><span class="status-chip">Custody path repaired</span></p></header>
...
"""
    ...
    agents_implementation_data = {
        ...
        "instrument_id": "discoverybench-agents-v1",
        "owner": "DD-010",
        "status": "implementation-and-custody-path-repaired-all-three-pilots-quarantined",
