...
def _benchmark_pages(root: Path, output: Path) -> dict[str, object]:
    ...
    agents_body = f"""
<header class="page-hero"><p class="eyebrow">DD-010 instrument · four quarantined pilots · agent protocol policy gated</p><h1>DiscoveryBench Agents v1</h1><p class="lede">A complete instrument for studying how software-agent teams turn synthetic evidence into action portfolios. Its original sealed pilot and three fresh repair-confirmation campaigns are permanently quarantined, while public provider-schema, semantic-contract, prospective custody-path conformance, and the AO-0009 aggregate diagnosis are complete. The current fail-closed agent protocol remains unchanged pending a separate policy decision. This is not a scientific evaluation or provider ranking.</p><p class="status-row"><span class="status-chip">Original pilot quarantined</span><span class="status-chip">Repair-confirmation v1 quarantined</span><span class="status-chip">Repair-confirmation v2 quarantined</span><span class="status-chip">Repair-confirmation v3 quarantined</span><span class="status-chip">Provider schemas passed</span><span class="status-chip">Custody path repaired</span><span class="status-chip">Agent protocol policy gated</span></p></header>
...
<section class="content-section prose"><h2>AO-0009 aggregate fixed-batch diagnosis</h2><p>The exact R2 owner-gated diagnostic ran once, authenticated all {int(agents_fixed_batch_outcome["diagnostic"]["authenticated_fixed_batch_traces"])} fixed-full-batch traces, verified the output lock, immutable inventory, append-only ledgers, complete response identity correspondence, unique 2/50/450 trace partition, all 500 pairing records, and retained-state immutability, then permanently closed private-read authority.</p><p>The public aggregate reports {int(agents_fixed_batch_outcome["aggregate_counters"]["protocol_nonconformance_traces"])} protocol-nonconforming traces and {int(agents_fixed_batch_outcome["aggregate_counters"]["parse_or_schema_repair_exhaustion_traces"])} parse/schema-repair exhaustion traces. Terminal-provider attempts, contamination traces, invalid final cardinalities, cap triggers, and integrity failures are zero. Frozen causal precedence selects <code>{html.escape(str(agents_fixed_batch_outcome["decision"]))}</code>.</p><p>AO-0009 makes no protocol acceptance, normalization, retry, invalid-output credit, or scoring change. Any prospective agent-protocol change requires a separate explicit owner decision and task. AO-0008 remains permanently quarantined, private-read authority is closed, and v4 is not registered.</p></section>
...
<section class="content-section prose"><h2>Versions and next gate</h2><dl><div><dt>Owner</dt><dd>DD-010 instrument</dd></div><div><dt>Content</dt><dd>Explicit selection from preserved v1, v2, or v3</dd></div><div><dt>Agent protocol</dt><dd><code>{html.escape(str(agents_versions["axes"]["agent_protocol"]))}</code></dd></div><div><dt>Generator</dt><dd><code>{html.escape(str(agents_versions["axes"]["task_generator"]))}</code></dd></div><div><dt>Original pilot lock</dt><dd><code>{html.escape(str(agents_pilot["output_lock_commitment"]))}</code></dd></div><div><dt>Repair-confirmation v1 lock</dt><dd><code>{html.escape(str(agents_fresh_closeout["lock"]["output_lock_commitment"]))}</code></dd></div><div><dt>Repair-confirmation v2 lock</dt><dd><code>{html.escape(str(agents_fresh_v2_closeout["lock"]["output_lock_commitment"]))}</code></dd></div><div><dt>Repair-confirmation v3 lock</dt><dd><code>{html.escape(str(agents_fresh_v3_closeout["lock"]["output_lock_commitment"]))}</code></dd></div></dl><p>The custody path is prospectively repaired and the fail-closed agent protocol remains unchanged. No replacement private pilot or base campaign is selected. Any agent-protocol change requires a separate explicit owner decision and task. Any future private evaluation additionally requires wholly new identities and a new exact owner gate. The 200-task base campaign remains blocked.</p><p><a href="../data/benchmark/agents-v1-provider-schema-conformance.json">Download the provider-schema conformance summary</a> · <a href="../data/benchmark/agents-v1-evaluation.json">Download the redacted operational summary</a> · <a href="../data/benchmark/agents-v1-implementation.json">Download the implementation summary</a> · <a href="../data/benchmark/agents-v1-registration.json">Download the preserved instrument registration</a> · <a href="../research/dd-010.html">DD-010 study record</a> · <a href="../benchmark.html">DiscoveryBench overview</a></p></section>
"""
    _write_benchmark_route_pair(
        output,
    ...
    agents_implementation_data = {
        ...
        "next_gate": (
            "explicit owner decision and separate task required for any agent-protocol change; "
            "no replacement private pilot selected"
        ),
    }
    _write(
        output,
